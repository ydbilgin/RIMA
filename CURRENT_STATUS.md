...
3. **⏭️ YENİ SESSION İLK İŞLER:** (a) T5 ax çıktısını QC+commit (`_done_T5_chamber.md` bekle — ax bağımsız koşuyor olabilir); (b) **T8 dispatch** (görev dosyası HAZIR: `_ax_task_T8_void_wiring.md` — T5 bitmeden BAŞLATMA, Unity çakışır); (c) Sunum Çekim Rehberi (FLOW_MAP + 05_DEMO_PRESENTATION_FLOW'dan derle) + Tool Audit (ax-3.1-Pro) — kuyruk sonu.
