# TASK: Generate ONE image with the image generation tool

ACTIVE RULES: (1) think before coding (2) min code, no speculation (3) surgical — listed files only (4) BLOCKED if unclear.

## Amaç
Use your built-in image generation capability (the `image_gen` / `$imagegen` tool) to render ONE image, then save it to an exact absolute path. This is an image-engine comparison test — do NOT write code, do NOT improvise with SVG/HTML.

## Prompt (render exactly this)
