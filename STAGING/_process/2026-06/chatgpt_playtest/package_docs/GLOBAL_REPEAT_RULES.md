...
- **Brawler Wall Slam Combo:** must be slide + VFX + hit-react only. Fallback when no wall (Ground-Slammed) required.
- **Brawler Pin Strike:** freeze max 0.8s normal / 0.3s boss; no bespoke pinned anim.
...
### Later (Deferred R4)
- Brawler Wall Slam Combo (collision/layout high-risk)
