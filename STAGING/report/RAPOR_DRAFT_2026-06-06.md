...
Sayısal olarak: 26 oda şablonu, 10 oynanabilir sınıf, 111 kayıtlı skill (~67'si çalışır implementasyona sahip), yaklaşık 490 EditMode + 39 PlayMode test tanımı (toplam ~529). Geliştirme sürecinde 100'ü aşkın karar belgesi yazıldı, her büyük mimari veya tasarım kararı belgelenerek arşivlendi. Bunlar yalnızca rakamlar değil; projenin mühendislik disipliniyle yönetildiğinin izleri.
...
**Dallanan Kapılar:** Oda temizlendikten ve ödül alındıktan sonra arka duvarda 1 ila 3 arasında kapı açılır. Her kapının üzerinde yer alan simge, hedefteki oda türünü işaret eder: olağan bir savaş mı, daha zorlu ama daha iyi ödüllü bir elite odası mı, yoksa doğrudan bir ödül odası mı? Oyuncu hangi kapıdan geçeceğine karar vererek run'un kısa vadeli akışını biçimlendirir (bkz. Şekil 5). Tehlikeyi mi yoksa güvenliği mi tercih eder; bu tercihler run'un sonundaki build'i doğrudan şekillendirir.
...
[Şekil 12: Attunement Chamber sahnesi — 10 sınıf istasyonu, mor void arka plan, cyan rün detayları]
...
- **10 oynanabilir sınıf** — her biri farklı kaynak ekonomisi ve oynanış diliyle
- **~529 test tanımı** (~490 EditMode + ~39 PlayMode) — kayıtlı koşuda 410 PASS
- **80 karakter sprite** — PixelLab ile üretilmiş, Point/PPU64 import sözleşmesiyle
- **8-sekmeli Map Designer** ve Room Browser/RoomJsonImporter araç zinciri
...
**Ses ve müzik.** Projede `AudioManager` bileşeni mevcuttur ve `RuntimeInitializeOnLoadMethod` ile kendiliğinden yüklenerek prosedürel yer tutucu SFX'ler çalmaktadır; kaynak dosyalardan override desteği de bulunmaktadır. Ancak prodüksiyon ses varlıkları ve orijinal oyun müziği henüz tamamlanmamıştır. Combat geri bildirimi (darbe, knockdown, yetenek sesleri), oda atmosferi ve run'a göre değişen müzik katmanları oyun hissini temelden etkiler. Bu alan, prototipten oynanabilir bir ürüne geçişin görünmez ama kritik bileşenidir.
...
| Şekil 12 | Attunement Chamber sahnesi — 10 sınıf istasyonu, mor void arka plan, cyan rün | 02_chamber_overview.png |
