...
- [4. Görsel Üretim Hattı](#4-görsel-üretim-hattı)
  - 4.5 Demo'da Kullanılan Görsel Asset Envanteri
...
Bu çalışma, tek geliştirici tarafından yapay zekâ destekli çok-ajanlı bir geliştirme süreci ve veri-güdümlü oyun mimarisi kullanılarak hayata geçirilen 2D izometrik bir aksiyon-roguelite oyunu olan RIMA'yı sunmaktadır. Projenin temel sorusu şudur: tek kişilik bir ekip, kapsamlı bir roguelite oyununu nasıl üretebilir? Bu proje, büyük dil modellerini tekil bir "komut ver-çalıştır" aracı olarak değil; rol bazlı, doğrulama döngülü ve kalıcı bilgi tabanına sahip bir agentic AI (etmen-temelli yapay zekâ) geliştirme sistemi olarak konumlandırır.
...
Proje; veri modeli on sınıfı destekleyecek şekilde kurulmuş olup 4 sınıfın çalışan controller implementasyonu bulunmaktadır (Warblade, Elementalist, Ranger, Shadowblade); demo'da Warblade ve Elementalist varsayılan açık ve uçtan uca oynanabilir, diğer ikisi kilitli olup aynı veri hattı üzerinden etkinleştirilebilir durumdadır; 26 oda şablonu, 549 test tanımı envanteri (508 EditMode + 41 PlayMode; son kayıtlı EditMode koşusu 410 PASS / 0 FAIL / 1 inconclusive) ve MainMenu'den boss karşılaşmasına uzanan oynanabilir tam döngüyle sonuçlanmıştır. Test altyapısı EditMode ve PlayMode testlerine ek olarak görsel oda kalite güvencesi sürecini kapsamakta; bu süreç gerçek hataları sistematik biçimde tespit etmiş ve düzeltmiştir.
...
The project delivers a playable end-to-end loop spanning MainMenu, a walkable diegetic character selection space (Attunement Chamber), room-by-room combat with 3-card skill drafts, branching Rift portal choices, and a boss encounter, supported by a ten-class data model (4 classes have working controller implementations: Warblade, Elementalist, Ranger, and Shadowblade; Warblade and Elementalist are unlocked by default and fully playable end-to-end in the demo build; the remaining two are locked and ready to enable via the same pipeline), 26 room templates, and a test inventory of 549 definitions (508 EditMode + 41 PlayMode; last recorded EditMode run: 410 PASS / 0 FAIL / 1 inconclusive). The quality assurance infrastructure extends beyond unit testing to include a programmatic visual room QC process, which identified and resolved systematic errors in prop placement across multiple room templates.
...
Bu proje, ticari ölçekteki bu örneklerin kapsamını birebir hedeflemiyor; referans oyunların ticari ölçeğini değil, temel mekanik prototipini hedefleyerek aynı türün temel döngülerini tek geliştirici ölçeğinde veri-güdümlü ve doğrulanabilir biçimde hayata geçirmeyi amaçlıyor. RIMA; on sınıflık bir veri altyapısı (4 sınıfın çalışan controller implementasyonu mevcut: Warblade, Elementalist, Ranger, Shadowblade; demo'da Warblade ve Elementalist varsayılan açık ve uçtan uca oynanabilir), sınıfa özgü kaynak ekonomileri ve 80'den fazla yetenek içeren gerçek bir skill draft sistemi, dal dallanmasıyla şekillenen bir dungeon graph, prosedürel içerik için geliştirilmiş bir araç zinciri ve bunların tümünü birbirine bağlayan oynanabilir tam döngüyle tasarlandı. Bütün bunları tek bir geliştirici yapıyordu.
...
**İkinci karar: yapay zekâ destekli çok-ajanlı geliştirme süreci.** Yapay zekâ entegrasyonunun kapsamını netleştirmek adına bir ayrım yapılmalıdır: yaygın olan "bir dil modeline komut ver, çalıştır" yaklaşımı bu projede benimsenen sistem değildir. Bu proje, büyük dil modellerini rol bazlı, doğrulama döngülü ve kalıcı bilgi tabanına sahip bir agentic AI (etmen-temelli yapay zekâ) geliştirme sistemi olarak konumlandırır.
...
Etmen-temelli yapay zekânın nasıl çalıştığını anlamak başlı başına bir öğrenme süreciydi; projenin ilk dönemlerinde tek bir modele iş yaptırmaktan çok-etmenli bir iş akışı tasarlamaya geçişin yöntem arayışıyla geçildi. Bu sürecin kuralları, ajanların kendiliğinden ortaya çıkardığı bir şey değil; denemeler ve gözlemlenen hatalardan öğrenilerek geliştirici tarafından tasarlandı ve zamanla rafine edildi. Ajanlar araçtır; süreci tasarlayan ve yöneten kişi geliştiricinin kendisidir.
...
Sayısal olarak: 26 oda şablonu, veri modeli 10 sınıfı ve geniş bir skill havuzunu destekleyecek şekilde kurulmuştur; 4 sınıfın çalışan controller implementasyonu mevcuttur (Warblade, Elementalist, Ranger, Shadowblade) ve demo'da Warblade ile Elementalist varsayılan açık ve uçtan uca oynanabilir durumdadır; kalan sınıflar aynı veri hattı üzerinden etkinleştirilebilir; 111 kayıtlı skill (~67'si çalışır implementasyona sahip), 549 test tanımı envanteri (508 EditMode + 41 PlayMode; son kayıtlı EditMode koşusu 410 PASS / 0 FAIL / 1 inconclusive). Geliştirme sürecinde 100'ü aşkın karar belgesi yazıldı, her büyük mimari veya tasarım kararı belgelenerek arşivlendi. Bunlar yalnızca rakamlar değil; projenin mühendislik disipliniyle yönetildiğinin izleri.
...
| **Harita dallanması** | Sınırlı çatallanma | Her odadan birkaç yol, biyom seçimi | StS-tipi ağaç harita, tip etiketleri | 1–3 dallı Rift portalı; her portal farklı oda türünü işaret eder |
...
[Şekil 1: Attunement Chamber — yürünebilir karakter seçimi (oda, pedestal'lar üzerinde sınıf silüetleri, [G] bürünme etkileşimi) | chamber.png *(kullanıcı verecek)*]

RIMA'nın karakter seçim ekranı, pek çok roguelite oyununun aksine statik bir menü değildir. Oyuncu, son oynadığı sınıfın bedeninde gerçek bir odaya spawn olur ve WASD tuşlarıyla serbestçe dolaşır (bkz. Şekil 1). Odanın çevresi boyunca pedestal'lar üzerinde on farklı sınıf, donmuş "echo"lar gibi bekledirilir: kilidi açılmış sınıflar taş-grisi tonda görünürken, henüz açılmamış olanlar opak siyah silüetler olarak durur. Bu ayrım hem görsel hem de anlatısal bir işlev görür; kaybedilmiş kimliklerin henüz yeniden kazanılmamış olduğu fikri, tek bakışta okunabilir.

Bir silüete yaklaştığında, o sınıfın yeteneklerini ve kaynağını özetleyen paneller ekranın kenarından kayarak girer. Oyuncu oradaki combat dummy (eğitim mankeni) üzerinde sınıfın vuruş hissini gerçek zamanlı olarak deneyebilir — seçim yapmadan önce nasıl bir karakter oynayacağını somut biçimde anlayabilir. [G] tuşuna basıldığında karakterin bedeni cyan bir enerji emilimi efektiyle o sınıfa dönüşür; hikâyesel olarak bu, unutulmuş bir kimlik yüzünü yeniden giymektir. Seçimi onaylamak için kuzey duvarındaki Rift kapısından yürüyerek çıkmak yeterlidir.
...
**Dallanan Rift Portalları:** Oda temizlendikten ve ödül alındıktan sonra arka kenarda (back edge) 1 ila 3 arasında Rift portalı açılır (kod mimarisinde bu çıkış noktaları `door_NW_01 / door_N_01 / door_NE_01` soket kimlikleriyle temsil edilir). Her Rift portalının üzerinde yer alan simge, hedefteki oda türünü işaret eder: olağan bir savaş mı, daha zorlu ama daha iyi ödüllü bir elite odası mı, yoksa doğrudan bir ödül odası mı? Oyuncu hangi Rift portalından geçeceğine karar vererek run'un kısa vadeli akışını biçimlendirir; tehlikeyi mi yoksa güvenliği mi tercih eder; bu tercihler run'un sonundaki build'i doğrudan şekillendirir.
...
**Oda kimliği göstergesi.** Oyuncunun run içinde nerede olduğunu her an okuyabilmesi için HUD'un üst-orta bölgesinde kalıcı bir oda etiketi gösterilmesi tasarlanmıştır: "ODA n/6 — TİP" biçiminde bir metin, mevcut `roomNameLabel` alanı yeniden kullanılarak çerçevelenecektir. Tip bilgisi SAVAŞ, TÜCCAR veya BOSS olarak gösterilecek; odaya girişte ise aynı etiket 1,5 saniyelik büyütülmüş bir giriş banner'ı olarak ekranın üst üçte birinde belirecek ve sonra normal boyutuna çekilecektir. Bu iki katmanlı kimlik gösterimi — anlık banner artı kalıcı etiket — yol haritasındaki bir tasarım kararı olup oyuncuya "neredeyim" bilgisini hem giriş anında hem sürekli olarak sunmayı hedefler.
...
RIMA'nın tüm yetenekleri merkezi bir SkillDatabase'de tutulmaktadır. Bu veritabanında toplam 111 skill kaydı mevcuttur; bunların 67'si oynanabilir ve çalışır durumda implementasyona sahipken, 44'ü tasarım aşamasında olan placeholder olarak beklemektedir. Placeholder kayıtlar SkillDatabase'de tasarım envanteri olarak tutulur ve Codex UI ekranında tüm 111 skill'i (implemented veya değil) görüntüler; ancak run içi draft havuzuna girmezler. `SkillDatabase.cs:580-587` satırlarındaki `GetPool` yöntemi `!isImplemented` filtresini uygular: `OfferGenerator` yalnızca implementasyonu tamamlanmış ve sınıf filtresinden geçen skill'leri çeker; implementasyonsuz bir skill hiçbir koşulda teklif listesine eklenemez. Bu ayrım, Codex ekranının (tam envanter görünümü) ile draft havuzunun (yalnızca oynanabilir skill'ler) kasıtlı olarak farklı davrandığı anlamına gelir. Veri modeli on sınıfı ve geniş bir skill havuzunu destekleyecek şekilde kurulmuştur; 4 sınıfın çalışan controller implementasyonu mevcuttur (Warblade, Elementalist, Ranger, Shadowblade) ve demo'da Warblade ile Elementalist varsayılan açık ve uçtan uca oynanabilir durumdadır; kalan sınıflar aynı veri hattı üzerinden etkinleştirilebilir. On sınıfın her biri farklı bir kaynak mekanizmasına ve farklı bir oynanış diline kurulmuştur; Rage barını hasar vererek dolduran savaşçıdan, combo zincirleri üzerine inşa edilmiş hızlı dövüşçüye, elementel yıkım üretmeye odaklanan büyücüden yavaş birikim ve ani patlama döngüsüne dayanan diğer sınıflara kadar her biri ayrı bir oyun tarzını temsil eder.
...
**Skill bar ilerleme tasarımı.** Draft akışının bütüncül bir anlatıya oturması için slot doldurma sırası da tasarlanmıştır. Run boş bir loadout'la başlar; ilk oda kurulur kurulmaz sınıfa özgü sabit içerikli bir açılış draft'ı açılır (Hades'in keepsake seçim anını andıran bir niyet bildirimi olarak tasarlanmıştır) ve seçilen skill Q slotuna yerleşir. Sonraki oda-temizleme draft'ları sırasıyla E, R ve F slotlarını doldurur; boss karşılaşmasına girişte oyuncu dört birincil slotu dolu bir kitle sahip olur. Boss sonrasında ise iki yedek slot (Z ve X) kilit açılarak fazladan iki skill alınabilir hale gelir. Bu yapı, skill bar'ı bir envanter listesi olarak değil, run boyunca büyüyen bir güç eğrisi olarak sunar ve oyuncunun her aşamada nerede olduğunu görsel olarak okumasını kolaylaştırır.

#### 2.4.2 Warblade Örneği — Bir Sınıfın Tasarım Dili

[Şekil 2: Warblade karakteri — omuz zırhı ve iki elli kılıç silueti | fig06_warblade.png]
...
[RewardTaken]
   │
   ▼  (Rift portalı veya portalları açılıyor)
[DoorOpen]
   │
   ▼  (oyuncu bir Rift portalına giriyor)
[Advancing]
   │             │
...
**Ses entegrasyonu.** Ses mimarisi iki katmanlıdır. Demo katmanı: CC0 lisanslı kliplerle entegre edilmiş temel SFX geri bildirimi — 18 adet CC0 lisanslı ses klibi projeye dahil edildi (kaynak: Kenney ses paketi, lisans dosyasıyla birlikte). Dokuz SFX kanalı tanımlandı: swing, hit, death, execute, oda-clear, skill draft, ambient, knockdown ve genel UI. Her kanal bağımsız volume kontrolüne sahiptir; sahne geçişlerinde ambient ses sızıntısını önleyen bir yaşam-döngüsü koruması da bu entegrasyon sırasında eklendi. Prodüksiyon katmanı: özgün müzik ve sınıfa özgü prodüksiyon-kalitesi SFX gelecek çalışma kapsamındadır.
...
**_Arena HUD bootstrap.** Oyunun ana savaş sahnesi olan `_Arena`'da HUD bileşenlerinin güvenilir biçimde ekrana gelmesi için bir bootstrap mekanizması eklendi. `RoomRunDirector` run başlatılmadan önce `HUDController.Instance`'ı sorgular; örnek yoksa çalışma zamanında `ScreenSpaceOverlay` canvas üzerinde otomatik kurulum gerçekleştirilir (sortingOrder=50, oyun-üstü UI katmanlarının altında). Aynı bootstrap, HP bar, resource bar ve skill bar'ın doğru canvas'a bağlandığını da doğrular. Bu yaklaşım, HUD'un sahneye elle yerleştirilmesini zorunlu olmaktan çıkarır ve içerik eklendiğinde altyapının her zaman hazır olmasını güvence altına alır.

**Skill bar sürükle-bırak yeniden sıralama.** Run içinde build tercihleri netleştikçe oyuncu skill'leri tercih ettiği tuşlara yeniden konumlayabilir: bir slot'a basılı tutup sürüklemek iki slot'un skill'lerini yer değiştirir; yeni konumun tuşu otomatik olarak geçerli hale gelir. Swap işlemi yalnızca görsel bir taşıma değil, `SkillController`'ın `slots[]` dizisine doğrudan yazılan kalıcı bir değişikliktir — input sistemi slot dizisinden okuduğundan tuş-skill eşlemesi anında güncellenir. Sürükleme sırasında kaynak slot %50 alfa ile soluklaşır, hedef slot cyan hale getirilerek bırakma noktası netleştirilir; geçersiz bir konuma bırakılan skill animasyonlu olarak eski yerine döner. LMB ve RMB slotları (temel saldırı ve sınıf-ikincili) sürükleme dışında tutulur; kimlik slotlarının yanlışlıkla taşınması engellenir.

**VFX-tabanlı skill sunumu.** Savaş okunabilirliği için karakter animasyon sprite'ları yerine VFX ve kod-tween kombinasyonunun kullanılması tasarlanmıştır. Her skill; bir çıkış ilkel hareketi (lunge, squash-stretch veya recoil tween'i), mevcut sprite varlıklarından çekilen bir VFX efekti ve `Assets/Scripts/Combat/Juice/` dizinindeki hazır sürücülere bağlanan bir impact reçetesiyle (hit-pause şiddeti + ekran sarsıntısı profili + HitFlash) tanımlanır. Bu yaklaşım sıfır yeni sprite üretimi gerektirmekte; mevcut VFX envanteri (slash_arc, glacial_spike, frozen_orb, meteor, fireball 8 yön) ve kod sürücüleri yeniden kullanılmaktadır. Prodüksiyon animasyon sprite'ları tamamlandığında bu reçeteler yerini zamanında değiştirilebilir; ancak demo sürecinde karakter animasyonu olmadan da skill'lerin eylem anı okunur biçimde sunulması bu tasarımın temel hedefidir.
...
Görsel yapının yanı sıra adanın fiziksel sınırlarının da tutarlı biçimde zorlanması gerekir. Bu katman üç boyutuyla ele alınmaktadır: **oda geometrisi** (statik sınır doğruluğu), **hareket güvenliği** (dinamik kenetleme) ve **doğrulama** (analiz ve test).
...
Oda tipine göre prop görünürlüğü de bilinçli biçimde farklılaştırılmıştır. Combat ve Elite oda tiplerinde prop'lar çalışma zamanında devre dışı bırakılarak temiz zemin ve uçurum sunulur; bu, savaş okunabilirliğini artıran ve oyuncunun spawn noktaları ile düşman konumlarını hızla okuyabilmesini sağlayan bir tasarım tercihidir. Prop otomatik yerleştiricisi tüm oda tipleri için bake işlemini gerçekleştirmeye devam etmekte; combat ve elite tipler ise bu bake edilmiş konumları çalışma zamanında yok saymaktadır.
...
[Şekil 3: Map Designer 8-sekmeli pencere görünümü | 11_map_designer.png]
[Şekil 4: Room Browser + _Arena'da kurulu oda | 12_room_browser_scene.png]
...
Bu sistemin tasarım değeri iki katmanda ortaya çıkar. Birincisi canonical kaynak netliği: ScriptableObject, oyun içi tek gerçek kaynak konumundadır; JSON ise dışa aktarım ve araç-entegrasyon formatı olarak tanımlanmıştır. Görsel editör, importer ve test hattı aynı veriyi iki ayrı gerçeklik gibi taşımaz. İkincisi araç-süreç bütünleşmesi: bir oda düzenlemek, dışa aktarmak ve test etmek tek bir iş akışı içinde gerçekleşir; geliştirici ile QC süreci arasında veri tutarsızlığı oluşamaz.
...
Bu bölüm, demo yapısında gerçekten etkin olan sprite ve VFX varlıklarını gruplar; §4.2–4.4'te anlatılan üretim süreçleriyle örtüşen yerlerde tekrar etmekten kaçınılmıştır.

#### Skill VFX — Sınıf Efektleri

Demo'da Warblade ve Elementalist sınıfları için VFX-tabanlı skill sunumu etkindir (commit `7d71a3ec`). Mevcut VFX envanteri: `slash_arc` (kılıç yay efekti), `glacial_spike`, `frozen_orb`, `meteor` ve fireball 8-yön sprite seti. Fireball, `create_8_direction_object` yöntemiyle üretilmiş ve tüm sekiz izometrik yönde bağımsız sprite olarak içe aktarılmıştır.

Her skill, bir hareket twini (lunge, squash-stretch veya recoil), mevcut VFX envanterinden çekilen efekt ve `Assets/Scripts/Combat/Juice/` sürücülerine bağlanan bir impact reçetesiyle (hit-pause şiddeti + ekran sarsıntısı profili + HitFlash) tanımlanır. Bu yaklaşım sıfır yeni sprite üretimi gerektirmeksizin skill eylemlerini görsel olarak okunur kılar. Karakter animasyon sprite'ları tamamlandığında bu reçeteler doğrudan o animasyon kaynağına geçiş yapabilir; mimari bu geçişe hazır şekilde tasarlanmıştır. Entegrasyon ayrıntıları §2.6'da ele alınmaktadır.

#### Act-1 Düşman Kadrosu — 12 Konsept Sprite

Demo'da aşağıdaki 12 düşman sprite'ı kullanılmaktadır. Tüm varlıklar ortak bir import sözleşmesini paylaşır: 64×64 piksel, PPU 64, Point filtre, sıkıştırma kapalı. Görsel stil: chibi high-top-down (~65° perspektif), 1 piksel outline, soğuk arduvaz-gri + cyan-Rift palet.

| Sprite | Açıklama |
|---|---|
| fracture_imp | Küçük hızlı melee düşman |
| relic_caster | Menzilli büyü saldırısı |
| seam_crawler | Zemin kıvrımından çıkan sürüngen |
| plate_widow | Zırhlı ağır yakın dövüşçü |
| hollow_arbitter | Orta menzilli okçu |
| rift_gound | Rift enerjisiyle yüklü savunmacı |
| spire_choirling | Grup destek büyücüsü |
| shard_walker | Kristal parça fırlatan melee |
| penitent_bruiser | Ağır yakın saldırı, yavaş |
| riftbound_augur | Rift portalı açabilen uzak saldırı |
| hollow_hulk | Büyük bodur tank; demo'da boss olarak da kullanılır |
| rift_acolyte | Zayıf ama çok sayıda gelen dalga birimi |

Demo boss'u, mevcut `hollow_hulk` sprite'ının büyütülmüş versiyonuyla temsil edilmektedir. Tasarım vizyonundaki nihai boss — "The Architect" — kendine özgü faz yapısı ve sprite setiyle yol haritasında yer almaktadır.

Tüm 12 mob sprite'ı, kaynaktan üç kat büyütülmüş (nearest-neighbor) ve etiketlenmiş biçimde aşağıdaki kontak sheet'te bir arada sunulmuştur (bkz. Şekil 5). `hollow_hulk` hücresi boss rolünü vurgulamak için ayrıca işaretlenmiştir.

[Şekil 5: Act-1 mob kadrosu kontak sheet'i — 12 düşman sprite, 4×3 grid; Hollow Hulk boss olarak işaretli | mob_roster_sheet.png]

#### Oynanabilir Sınıf Kadrosu — 10 Sınıf Dizilimi

RIMA'da kilit açılabilir on oynanabilir sınıf tanımlıdır. Her sınıf `Assets/Resources/Characters/<Sınıf>/<sınıf>_idle_south.png` yolunda güney yönüne bakan idle sprite'a sahiptir. Aşağıdaki kontak sheet bu on sprite'ı yan yana, etiketli biçimde göstermektedir (bkz. Şekil 6). Demo'da Warblade ve Elementalist aktif; Shadowblade ve Ranger teknik olarak oynanabilir; kalan altı sınıf kilit sistemiyle Attunement Chamber'da yer alır.

[Şekil 6: Oynanabilir sınıf dizilimi kontak sheet'i — 10 sınıfın idle_south sprite'ları, etiketli 2×5 grid | class_lineup_sheet.png]

#### Zemin ve Cliff — §3.2 Özeti

Zemin ve cliff asset'leri §3.2'de ayrıntılı olarak ele alınmıştır. Kısa referans: standart koyu granit zemin ve checker desen varyantı; cliff sprite'ları çalışma zamanında `IsoRoomBuilder` tarafından otomatik yerleştirilir; güney cephesindeki "içeri tuck" kuralı ada sınırı dışına taşmayı önler. Bu asset'ler için ek üretim bilgisi §3.2'de bulunmaktadır.
...
Bu bölümde açıklanan sistem, büyük dil modellerini tekil bir "komut ver-çalıştır" aracı olarak değil; rol bazlı, doğrulama döngülü ve kalıcı bilgi tabanına sahip bir agentic AI geliştirme sistemi olarak konumlandırır. Etmen-temelli yapay zekânın nasıl çalıştığını anlamak başlı başına bir öğrenme süreciydi; hızla evrilen bir alanın pratiklerini bir prototipin üretim sürecine oturtmak — ajan rolleri, yazar-gözden geçiren ayrımı, karar belgeleri ve doğrulama kanıtı zorunluluğunu — zamanla rafine edilen bir sisteme dönüştürdü. Bu yaklaşım, sektörde hızla olgunlaşan agentic AI pratiklerini akademik bir prototip üretimine uyarlama denemesidir.
...
| **Bilgi Tabanı** | NotebookLM (MCP üzerinden) | Tasarım kararlarının kalıcı, oturumlar arası sorgulanabilir arşivi |
| **Ortam Sürücüsü** | Unity MCP, PixelLab MCP | Ajanların doğrudan Unity editörünü ve görsel üretim hatlarını sürmesi |
...
Bilgi tabanı rolünü üstlenen NotebookLM, projenin belleği olarak işlev görmüştür. Tasarım kararları, oyun mekanik tartışmaları ve sistem spesifikasyonları buraya senkronize edilmiş; her ajan büyük bağlam okumaya gitmek yerine bu tabanı sorgulamıştır. Kritik nokta, bu erişimin Model Context Protocol (MCP) aracılığıyla gerçekleşmesidir: orkestratör ajan, NotebookLM'i bir harici araç olarak çağırır ve kanonikleri ister; cevap doğrudan görev bağlamına enjekte edilir. Bu düzenek hem token tüketimini azaltmış hem de kararların oturum başından oturum sonuna tutarlı kalmasını sağlamıştır.

MCP'nin rolü bilgi tabanıyla sınırlı kalmamıştır. Aynı protokol, ajanların Unity Editor üzerinde doğrudan işlem yapmasını da mümkün kılmıştır: sahne nesnesi oluşturma, bileşen ekleme, ScriptableObject varlığı güncelleme ve oynatma modu kontrolü MCP araçları aracılığıyla ajan tarafından yürütülmüştür. Benzer biçimde görsel üretim hattında PixelLab MCP entegrasyonu, sprite üretim görevlerinin (8 yönlü karakter, skill ikonu) bir ajan tarafından doğrudan tetiklenmesini ve sonuçların projeye aktarılmasını sağlamıştır. Bu iki entegrasyon, ajanın rolünü yalnızca metin üretmekten ortamda eylem gerçekleştirmeye taşımaktadır.
...
**MCP ile ortam erişimi.** Ajanın Unity Editor ve görsel üretim hattıyla etkileşimi saf metin alışverişinin ötesine geçmektedir. Model Context Protocol üzerinden bağlı araçlar sayesinde ajan; sahne nesnesini oluşturmak, ScriptableObject alanını güncellemek veya PixelLab'e yeni bir sprite işi göndermek için ayrı bir insanın manuel adım atmasını beklemez. Bu mekanizma, özellikle toplu içerik görevlerinde (oda varlığı oluşturma, 16 ikon yeniden içe aktarma, sprite pivot düzeltmesi) süreç hızını belirgin biçimde artırmıştır.
...
Daha geniş bir perspektiften bakıldığında, bu yaklaşım sektörde hızla olgunlaşan agentic AI pratiklerini akademik bir prototip üretimine uyarlama denemesidir. Araştırma gruplarının ve küçük stüdyoların çok-etmenli iş akışlarını benimsemeye başladığı bu dönemde, tek geliştiricinin benzer bir sistemi belgelenmiş biçimde işletmesi metodolojik bir katkı olarak değerlendirilebilir.
...
Sahne geçişleri sırasında oda temizleme akışı çalışmıyordu: düşmanların tamamı ortadan kaldırıldıktan sonra ne ödül beliriverdi ne de bir sonraki odaya geçişi sağlayan Rift portalları aktif hale geldi. Sistem herhangi bir hata mesajı üretmiyordu; oyun sessizce kilitleniyordu.
...
RIMA, oynanabilir tam döngüye sahip çalışan bir oyundur. Ana Menü'den başlayan, yürünebilir Attunement Chamber'da karakter seçimiyle devam eden, oda oda combat ve 3-kart skill draft içeren, dallanan Rift portallarıyla şekillenen ve boss karşılaşmasıyla kapanan bir run döngüsü — bunların tamamı birbirine bağlı ve çalışır durumdadır. Projenin somut çıktıları şu biçimde özetlenebilir:

- **26 oda şablonu** — 15 dışarıdan temin edilmiş, tamamı görsel QC'den geçirilmiş
- **10 sınıflık veri altyapısı** — her biri farklı kaynak ekonomisi ve oynanış diliyle tanımlanmış; 4 sınıfın çalışan controller implementasyonu mevcut (Warblade, Elementalist, Ranger, Shadowblade), demo'da Warblade ve Elementalist varsayılan açık ve uçtan uca oynanabilir, kalan sınıflar aynı pipeline üzerinden etkinleştirilebilir
...
**Ses ve müzik.** Demo kapsamında temel SFX geri bildirimi CC0 lisanslı kliplerle entegre edilmiştir; 18 adet CC0 lisanslı ses klibi ve 9 SFX kanalı bağlanmış, sahne geçişlerinde ambient ses sızıntısını önleyen yaşam-döngüsü koruması da bu aşamada eklenmiştir. Mevcut durumda ses geri bildirimi clear SFX ve görsel efektlere dayanmaktadır; özgün müzik ve sınıfa özgü prodüksiyon sesleri gelecek çalışma kapsamındadır. Bu alan, prototip kalitesinden çıkma sürecinin görünmez ama kritik bileşenlerinden biri olmaya devam etmektedir.

**Rift portalı görsel entegrasyonu ve boss geliştirme.** Portal soket sistemi ve görsel açı kararları tamamlandı; bir sonraki adım Rift portalı sprite'larının oyun içi görünümüne tam entegrasyonu ve mevcut boss arenasının görsel ile mekanik açısından derinleştirilmesidir. Demo kapsamında boss düğümüne ulaşan ve temel saldırı/zafer akışını tamamlayan bir boss karşılaşması bulunmaktadır; daha zengin faz yapısı ve telegraph çeşitliliği yol haritasına bırakılmıştır. Act 2 ve sonrasını mümkün kılmak için yeni boss tasarımları, biyom çeşitliliği ve artan zorluk eğrisi gerekecektir. Bu, projenin içerik derinliği açısından en uzun vadeli çalışmasıdır.
...
Bu proje; veri-güdümlü mimari, çok-ajanlı geliştirme süreci ve programatik kalite güvencesinin tek geliştirici ölçeğinde nasıl uygulanabileceğini belgelemektedir. Oynanabilir tam döngü, 10 sınıf, 26 oda şablonu ve 549 test tanımı envanteri (508 EditMode + 41 PlayMode; son kayıtlı EditMode koşusu 410 PASS / 0 FAIL / 1 inconclusive) ile ulaşılan kapsam, benimsenen mimari ve metodolojik kararların somut çıktısıdır. Projenin sürdürülmesi ve ticari kaliteye taşınması için gerekli yol haritası Bölüm 8.2'de özetlenmiş olup temel mühendislik altyapısı bu çalışmanın kapsamı dahilinde tamamlanmıştır.
...
| No | Açıklama | Dosya |
|---|---|---|
| Şekil 1 | Attunement Chamber — yürünebilir karakter seçimi | chamber.png *(kullanıcı verecek)* |
| Şekil 2 | Warblade karakteri — omuz zırhı ve iki elli kılıç silueti | figures_v2/fig06_warblade.png |
| Şekil 3 | Map Designer 8-sekmeli pencere görünümü | report_screenshots/11_map_designer.png |
| Şekil 4 | Room Browser + _Arena'da kurulu oda | report_screenshots/12_room_browser_scene.png |
| Şekil 5 | Act-1 mob kadrosu kontak sheet'i — 12 düşman sprite, 4×3 grid; Hollow Hulk boss olarak işaretli | figures_v2/mob_roster_sheet.png |
| Şekil 6 | Oynanabilir sınıf dizilimi kontak sheet'i — 10 sınıfın idle_south sprite'ları, etiketli 2×5 grid | figures_v2/class_lineup_sheet.png |
