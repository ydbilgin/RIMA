...
RIMA (Rift Avcıları), tek bir geliştirici tarafından yapay zekâ destekli çok-ajanlı bir geliştirme süreci ve veri-güdümlü bir oyun mimarisi kullanılarak hayata geçirilen 2D izometrik bir aksiyon-roguelite oyunudur. Oyuncu, kendine özgü güçlere ve birbirinden tamamen farklı kaynak ekonomilerine sahip savaşçı sınıflarından birini seçerek prosedürel olarak ilerleyen bir zindan koridorunda oda oda dövüşür; her geçişte yeni yetenekler edinerek daha güçlü bir yapı (build) inşa eder. Başarısız bir koşu (run) dahi sonraki oturumlar için kalıcı bir kazanım bırakır.
...
- **AI-destekli geliştirme katmanı:** Çok-ajanlı orkestrasyon (Kod Üretim Modülü ve Mimari Çapraz-Doğrulama kanalları¹), Model Context Protocol (MCP) üzerinden Unity Editor ve PixelLab sürümü, kalıcı bilgi tabanı için NotebookLM, kod-grafı analizi için graphify.
...
### 2.2 Proje Klasör Yapısı ve Sınıf Sorumlulukları

Tüm oyun kodu `Assets/Scripts/` altında, sistem sorumluluklarına göre ayrılmış modüler bir klasör ağacında toplanmıştır. Aşağıdaki ağaç, kod tabanının küratörlü bir görünümüdür; her modül tek bir mimari katmana karşılık gelir:

```
Assets/Scripts/
  MapDesigner/   oda sistemi: IsoRoomBuilder, RoomRunDirector, RoomTemplateSO, prop yerleşimi
  Skills/        yetenek/kart sistemi: SkillBase, sınıf SkillController'ları, DraftManager, cross-class
  Systems/       çekirdek oyun sistemleri ve singleton yöneticiler
  Combat/        dövüş, hasar, stat, game-feel (juice) sürücüleri
  UI/            BuildModeController, DirectorMode, HUD, SkillBarUI, tooltip
  Core/          RewardPickup ve çekirdek run akışı
  Environment/   çevre, Light2D ışık, prop tanımları
  Enemies/       düşman AI (BaseMobBehavior) ve dalga/spawn
  Runtime/       runtime orkestrasyon ve kayıt (registry) baker'ları
  Editor/        Unity editör araçları (RoomJsonImporter, Map Designer vb.)
  Data/ Player/ Map/ VFX/ Audio/   veri varlıkları, oyuncu, harita yardımcıları, efekt, ses
```

Kod-grafı analizinde (bkz. Bölüm 10) en yüksek bağlantı derecesine sahip "god-node" sınıflar, sistemin etrafında döndüğü merkezlerdir. Bu sınıfların sorumlulukları ve etkileşimleri şu hiyerarşiyle özetlenebilir:

- **`RoomRunDirector`** (oda yaşam-döngüsü orkestratörü). Bir run boyunca odaların yaşam döngüsünü (build → combat → cleared → reward → door → advancing) yedi durumlu bir durum makinesiyle yönetir. `IsoRoomBuilder`'a hangi şablonun kurulacağını söyler, `EncounterController`'dan "temizlendi" sinyalini bekler, ödül ve portal akışını tetikler; üst düzeyde `DungeonGraph`'tan gelen run-map'e göre bir sonraki odayı seçer. Oyun-tarafının merkezî koordinatörüdür.
- **`IsoRoomBuilder`** (veri → sahne inşa motoru). Salt-veri olan `RoomTemplateSO`'yu okuyup zemin karolarını, otomatik uçurum (cliff) sprite'larını ve bake edilmiş prop'ları tek bir `_Arena` sahnesine yerleştirir. `RoomRunDirector` tarafından her oda geçişinde çağrılır; veri katmanı ile çalışan sahne arasındaki köprüdür.
- **`SkillController` (sınıf-bazlı aile)**. Yetenek davranışı, ortak bir `SkillBase` soyut sınıfı ve her oyuncu sınıfı için ayrı bir controller (`Warblade_SkillController`, `Elementalist_SkillController`, `Ranger_SkillController`, `Shadowblade_SkillController`) ile gerçeklenir. Controller, `SkillData` veri kayıtlarını çalışma-zamanı eylemine bağlar; slot durumu `SkillBarUI` ile, draft akışı `DraftManager` ile etkileşir. Bu, "imza yetenekler bespoke kalır" tasarım disiplininin kod düzeyindeki karşılığıdır.
- **`BuildModeController` ve `DirectorMode`** (oyun-içi araç god-node'ları). `DirectorMode` çalışma zamanında sekmeli bir UI üreten geliştirici konsoludur (Stat/Spawn/Telemetry/Build/Map); kod-grafında en bağlı node'dur, çünkü hemen her sisteme canlı bir müdahale arayüzü sağlar. `BuildModeController`, `DirectorMode`'un toggle altyapısı üzerine oturan oyun-içi seviye editörünü (F2 / ") yönetir ve prop yerleştirmeyi `BuildPlacementController`'a devreder. Her iki sınıf da `#if DEMO_BUILD || DEVELOPMENT_BUILD || UNITY_EDITOR` koşuluyla derlenir; üretim sürümünden dışlanır.

Genel akış üç katmanlıdır: **veri katmanı** (ScriptableObject varlıkları — `RoomTemplateSO`, `SkillData`, `WeaponDatabaseSO`) içeriği taşır; **runtime yönetici katmanı** (`RoomRunDirector`, sınıf controller'ları, `Systems` üzerindeki singleton'lar) bu veriyi okuyup davranışa çevirir; **sahne katmanı** (`_Arena` ve kalıcı `Systems` nesnesi) çalışan oyunu barındırır. Araç sınıfları (`BuildModeController`, `DirectorMode`, editör araçları) bu üç katmanın aynısını tüketir; bir tasarım değişikliği hem oyun hem araç tarafında anında yansır.

### 2.3 Sahne Akışı ve Manager Katmanı
...
RIMA'nın karakter seçim ekranı statik bir menü değil, gerçek bir odadır. Oyuncu son oynadığı sınıfın bedeninde spawn olur ve WASD ile serbestçe dolaşır. Oda çevresindeki pedestal'larda sınıflar donmuş "echo"lar gibi bekler: açık sınıflar taş-grisi, kilitli olanlar opak siyah silüetlerdir. Bir silüete yaklaşıldığında sınıf paneli kayarak girer; oyuncu bir combat dummy üzerinde vuruş hissini deneyebilir. [G] tuşu karakteri o sınıfa dönüştürür; kuzey duvarındaki Rift kapısından çıkmak seçimi onaylar.

#### Editör Deneme Alanı (Chamber [K])

Attunement Chamber'da oyuncu `[K]` tuşuyla aktif sınıfın uygulanmış tüm yetenek listesine (`SkillDatabase`) erişir ve istediği yeteneği Q/E/R/F yuvalarından birine atayarak combat dummy üzerinde deneyebilir. Slot atamaları yalnızca oda-içi oturumda bellekte tutulur; run başladığında sıfırlanır ve koşu yükü ile draft akışına dokunmaz. Bu özellik, özellikle geniş yetenek havuzu olan sınıflarda oyuncunun run öncesinde sinerji olasılıklarını keşfetmesini sağlar (commit `981ac783`).

#### Dövüş (Combat)

Oda aktif olduğunda düşman dalgaları sahneye girer. Oyuncu sınıfa özgü saldırı zinciri (LMB combo) ve yetenek seti (Q/E/R/F) ile dövüşür. Düşmanlar hasar aldıklarında geri savrulur; ağır saldırılar tam bir knockdown animasyonu başlatır — düşman yere düşer ve kalkış animasyonu bitene kadar i-frame (geçici dokunulmazlık) korumasına sahiptir. Son düşman da devrildiğinde oda "Cleared" durumuna geçer.

#### 3-Kart Yetenek Seçimi (Skill Draft)

Her üç odada bir, oda ortasında bir ödül nesnesi belirir. [G] ile alındığında üç yetenek (skill) kartı açılır; her kart sınıfa ait ya da nötr bir pasif yetenek sunar ve araç ipucu (tooltip) mevcut yapıyla (build) sinerji noktalarını gösterir. Oyuncu birini seçer ve yetenek kalıcı olarak koşuya (run) eklenir. Bu akış Şekil 2'de gösterilmiştir.

[Şekil 2: 3-kart skill draft ekranı ve ödül akışı | fig_draft_reward.png]

#### Dallanan Rift Portalları
...
Oturum (run) sonuna doğru, harita grafiğinde boss düğümüne ulaşıldığında boss arenasına giden portal açılır. Boss arenası boyut ve zorluk açısından standart odalardan ayrışır. Demo kapsamındaki boss, mevcut `hollow_hulk` sprite'ının büyütülmüş versiyonuyla temsil edilir ve telegraph sistemiyle altı saldırıya bağlanmıştır (bkz. §4.4). Demo karşılaşmasının kod-tarafı adı Penitent Sovereign'dır (bkz. §11.8). Tasarım vizyonundaki nihai boss — "The Architect" — zengin bir faz yapısıyla yol haritasındadır.

#### Zafer / Ölüm ve Meta-İlerleme

Oturum sonunda oyuncuya kazandığı Shattered Echo miktarı gösterilir; hesap formülü `odaSayısı × 3 + öldürmeSayısı / 5` olup run başına 5–60 Echo ile sınırlıdır. Biriken Echo'lar bir sonraki oturumda kilitli sınıf silüetlerinden birini açmak için harcanır.
...
Warblade, "yaklaş, sabitle, zırh kır, infaz et" mantığıyla çalışan ağır bir savaşçıdır. Kaynağı, yalnızca hasar vererek ve kitle kontrolü uygulayarak dolan, savaş dışında pasif eriyen bir Öfke göstergesidir (Rage bar); bu, oyuncuyu sürekli temasa zorlar. Yetenek seti ("Iron Charge", "Gravity Cleave", "Iron Counter") savaşı düşmanı Broken veya Sundered durumuna yönlendirir; bu durumlardaki bir hedefe imza yetenek "Death Blow" uygulanabilir — Öfke göstergesini boşaltarak %400+ hasar veren bir infaz darbesi. Öfke göstergesi dolduğunda [V] ile "Bladestorm" ultimate devreye girer. Warblade sınıfının temel sprite'ı Şekil 3'te görülmektedir.

[Şekil 3: Warblade sınıfı — omuz zırhlı silahsız temel sprite; silah mount sistemi Şekil 9'da | figures_v2/fig06_warblade.png]
...
Düşman AI'sinin temelini `BaseMobBehavior` oluşturur. Bu sistem, bir combat-okunabilirlik vakasının da kaynağı olmuştur (ayrıntı §11.4): `detectionRange` algılama menzili üretim (spawn) mesafesinin altında kaldığında düşmanlar kalıcı boşta (Idle) durumunda kalıyordu; kök neden veri-güdümlü doğrulamayla bulunup düzeltilmiştir.

Boss telegraph sistemi, ARPG konvansiyonuna uygun olarak saldırı öncesi tehlike alanlarını görselleştirir. Mevcut motor yeniden kullanılarak altı boss saldırısına bağlanmıştır: HolyLash (koni), FractureStrike (daire), ChainExplosion (gecikmeli halka), SovereignsWrath (dış halka + yeşil güvenli-bölge), FractureCharge (atılım çizgisi) ve ShackleThrow. Windup zamanlaması saldırı anıyla bit-bit senkronizedir; telegraph-hasar uyumsuzluğu (görselden farklı zamanda vuran saldırı) önlenmiştir. SovereignsWrath'ın yeşil güvenli-halkası, tehlike halkasından ayrı renkte çizilerek oyuncuya "nereye durmalı" bilgisini okunur biçimde verir.
...
**Hareket güvenliği — yetenek tabanlı yer değiştirme.** Blink (ışınlanma), Iron Charge ve Blade Rush yetenekleri başlangıçta yürünebilirlik haritasını (`WalkabilityMap`) atlayarak oyuncuyu harita dışı geçersiz alana (void) gönderebiliyordu. Bu yetenekler, standart oyuncu hareketinde de kullanılan `WalkabilityMap.IsDashableWorld` / `ClampVelocityToWalkable` boru hattına bağlanarak ortak bir yürünebilirlik kapısından geçirildi; artık tüm yetenek tabanlı yer değiştirmeler oda sınırlarına saygı duyar (commit `6ba61ff5`).

### 4.6 Oyun Hissi (Game-Feel) Katmanı

Oyun hissi (game-feel) katmanı, bir aksiyon oyununun mekanik doğruluğunun ötesinde inandırıcı hissetmesini sağlar. RIMA'da bu katman ekran sarsıntısı, zaman dondurması ve ses geri bildiriminden oluşur.
...
**CombatJuice.** Hasar sayıları, hit-stop, ekran sarsıntısı ve kamera-punch bileşenlerinden oluşan `CombatJuice` katmanı `_Arena` sahnesine bağlanmış ve canlı doğrulanmıştır: vuruş → hasar sayısı, kill → freeze; `timeScale` değişimine bağışık, sahne-geçişinde sızıntısız ve Build Mode/Director geçişlerinde temiz. Bu katman kodda hazır olduğu hâlde sahneye bağlı değildi; keşfedilip aktive edilerek minimum kaynakla yüksek oyun hissi elde edilmesinin somut bir örneği oluşturulmuştur.
...
**Düşman okunabilirliği.** `EnemyReadable.shader`, herhangi bir renkte gerçek silüet outline üretir; ambient ışık 0.22'den 0.35'e çıkarılarak düşmanlar net biçimde öne çıkar, atmosfer korunur. Bu çalışmada gizli bir bug bulunmuştur: `EnsureVisibleSprite` her karede özgün materyali üzerine yazıyordu (clobber); eski dış çizgi hiç oluşturulmuyordu — kök-nedene inilip düzeltildi.
...
**Bekleme süresi sayısal göstergesi.** Skill çubuğunda aktif bekleme süreleri artık sayısal olarak görselleştirilir: ≥1 saniyede tam sayı, <1 saniyede ondalık değer. Önceki sürümde gösterge yalnızca ≥5 saniyelik eşikte devreye giriyordu; bu aralığın altındaki bekleme süreleri okunaksız kalıyordu. Güncelleme, oyuncunun tüm slot durumlarını anlık değerlendirerek saldırı planlamasını etkili şekilde yapabilmesini sağlar (commit `981ac783`).
...
**Görsel atmosfer — URP son-işleme katmanı.** Demo sunumu için URP Volume tabanlı son-işleme etkinleştirildi: Bloom (rift enerji parıltıları ve ayna yüzeyleri), Color Adjustments (pozlama/kontrast), Neutral Tonemapping ve Vignette efektleri; ek olarak global 2D ortam ışığı ve brazier objesine sıcak tonlu nokta-ışık atandı. "Yüzen ada + void" görsel kanonu korunarak sahnedeki obje ve karakterlerin okunabilirliği artırıldı (commit `9359b7a5`).

---

## 5 OYUN-İÇİ GELİŞTİRME ARAÇLARI

Projenin merkezî mühendislik katkısı, oyun çalışırken — Unity editörünü açıp kapatmadan — tasarım iterasyonu yapılmasını sağlayan oyun-içi araç katmanıdır. Bu araçlar graphify kod-grafı analizinde en bağlı (god-node) sınıflar arasındadır ve "bu proje bir geliştirme ortamıdır" tezinin somut kanıtıdır.

### 5.1 Build Mode (F2) — Edit-to-Play

Build Mode, oyun çalışırken `F2` tuşuyla açılan oyun-içi seviye editörüdür. Açıldığında oyun duraklatılır; geliştirici nesneleri (prop) yerleştirir, zemini düzenler, odayı kurar — sonra modu kapatıp **aynı odayı anında oynar**. Bu "düzenle-ve-oyna" (edit-to-play) döngüsü, geleneksel "Unity'de düzenle → kaydet → Play'e geç" döngüsünü ortadan kaldırır.

`BuildModeController` (graphify'da en bağlı araç düğümlerinden), tek-sahip bir `F2` kayıt defteri üzerinden çalışan geçiş mantığını, üzerinde çalışılan `WorkingTemplate`'i ve düzenle↔oyna geçişini yönetir. Prop yerleştirme `BuildPlacementController` tarafından bir palet → hayalet (ghost) → tıkla akışıyla gerçekleştirilir. Palet veri-güdümlüdür: `BuildModeAssetCatalog`, `PropRegistrySO`'dan beslenir; palet 9'dan 19 prop'a çıkarılmıştır. Bu araç, Bölüm 2'de tanımlanan veri-güdümlü mimarinin doğrudan bir tüketicisidir: aynı `RoomTemplateSO` verisini hem oyun hem editör paylaşır, dolayısıyla değişiklikler anında yansır. Şekil 4, Build Mode arayüzünü göstermektedir.
...
Director Mode (`DirectorMode.cs`), sahne kurulumu gerektirmeden çalışma zamanında bir arayüz üreten geliştirici konsoludur. Açıldığında sekmeli bir IDE-dock düzeninde çalışır ve şu panelleri sunar: Stat slider'ları (oyuncu/düşman istatistiklerini canlı ayarlama), Spawn (düşman üretimi), Telemetry (CSV çıktısı), Prop, Map ve Free-cam. Bu kapsamda Director Mode, eski hata-ayıklama katmanı görünümünden profesyonel bir dock düzenine (görüntü alanı ~%57, SDF yazı tipi, ScrollRect'li kaydırılabilir yetenek listesi) taşınmıştır. Director Mode, kod-grafında en yüksek bağlantı derecesine sahip düğümdür (bkz. Bölüm 10), çünkü oyunun hemen her sistemine canlı bir gözlem ve müdahale arayüzü sağlar. Şekil 5, Director Mode panellerini göstermektedir.

Araç sağlamlığı kapsamında çift-yuva reddi de eklendi: zaten bir yuvada bulunan yetenek bileşeni ikinci bir yuvaya atanmak istendiğinde, atama bileşeni oluşturulmadan önce reddedilerek mevcut hata-bildirim arayüzü üzerinden kullanıcı uyarılır; bu sayede aynı bileşenin iki yuva tarafından paylaşılıp bekleme sürelerini bozduğu öksüz bileşen sızıntısı önlendi.
...
Şekil 6, Act-1'in altı odasının `IsoRoomBuilder` ile çalışma zamanında inşa edilmiş oyun-içi görünümünü bir arada gösterir; `BuildCliffs`'in salt-veri taban hücrelerinden ürettiği prosedürel uçurum kenarı, harita dışı geçersiz alan (void) arka planı ve 2B ışık birleşince ada-görünümünün tümüyle veri + inşa düzeneğinden geldiği — elle yerleştirilmiş tek bir karo olmadan — doğrudan gözlemlenebilir.

[Şekil 6: Act-1'in 6 odası IsoRoomBuilder ile üretilen cliff-island oyun-içi görünümü — procedural uçurum kenarı + void arka plan + 2D ışık | figures_2026-06-18/fig_rooms_island_grid.png]
...
26 oda şablonunun yaklaşık 11'i dahili tasarımdır; 15'i prosedürel JSON oda şablonu olarak harici araçlarla üretilip içe aktarılmıştır. Süreç gözetimsiz bırakılmamıştır: geçerli bir tasarım, ASCII ızgara düzeni, kapı soketleri, zorluk etiketi ve prop listesi içeren belirli bir JSON şemasına uymalıdır; bu format doğrudan `RoomJsonImporter` aracına beslenebilir. Üretilen tasarımlar bir eleme sürecinden (çeşitlilik, zorluk dengesi, tematik uygunluk) geçirilmiş, seçilenler mevcut odalarla birleştirilerek 26 şablonluk havuz oluşturulmuştur.

| Oda Havuzu Özeti | Değer |
|---|---|
| Toplam oda şablonu | 26 |
| Dahili tasarım | ~11 |
| Harici araçla üretilip içe aktarılan | 15 |
| QC uygulanan şablon | 26 |

Bu odalar, harici araçlarla üretilen JSON şablonlardan `RoomJsonImporter` aracılığıyla içe aktarılır. Her oda dosyası zorunlu alanlar olan oda kimliği (`room_id`), ızgara boyutları (`width`/`height`), zemin malzeme bölgeleri (`floor.zones`) ve duvar prefab konumlarını içerir. Şema-doğrulamasından geçmeyen dosyalar içe aktarım aşamasında reddedilir.

| JSON Oda Şeması — Temel Alanlar | Açıklama |
|---|---|
| `schema_version` | Format sürümü (doğrulama için) |
| `room_id` | Benzersiz oda tanımlayıcısı |
| `room_type` | spawn / combat / elite / reward / boss |
| `width`, `height` | Izgara boyutları (hücre) |
| `floor.zones[]` | Malzeme bölgeleri: rect + blend_edges |
| `walls[]` | Prefab konumları: prefab adı + (x, y) + rotation |

Tekil oda dosyalarının yanında bir `_manifest.json`, odaların birbirine nasıl bağlandığını tanımlayan bir kapı grafiği (door_graph) taşır; bu graf doğrudan oturum-haritasının (run-map) dallanmasını besler. Giriş odası (entry_hall), Kuzey/Doğu/Batı dallarıyla altı-odalı bir bağlılık grafı oluşturur: entry_hall → north_antechamber → shattered_throne (dikey omurga); east_corridor → treasure_vault (doğu kolu); west_chamber → north_antechamber (döngüsel bağ). Bu veri katmanının şematik görünümü Şekil 7'de verilmiştir.

[Şekil 7: Aynı 6 odanın JSON→tilemap şematik veri katmanı (oda kimliği, boyut, zemin bölgeleri) | figures_2026-06-18/fig_rooms_grid.png]
...
Her sınıf sekiz yönde (K, KD, D, GD, G, GB, B, KB) idle sprite'a ihtiyaç duyar; toplam 80 sprite elle çizilemeyecek bir yüktür. Çözüm PixelLab AI'dir. Sekiz yönün tamamı bağımsız üretilmez: K, KD, D, GD, G beş yön yapay zekâ ile üretilir; GB, B, KB ise yatay aynalama (flipX) ile türetilir. Bu, içerik maliyetini yaklaşık %37 düşürür ve aynalanan yönlerde stil tutarlılığını doğal olarak korur. Tuval boyutları sınıf silüetine göre değişir (120×120, 124×124 veya 128×128); görünür gövde her sınıfta ~64 piksel uzunluğundadır, kalan boşluk animasyon eylemleri için tampondur. 10 sınıfın güney-yön sprite'larının yan yana dizilimi Şekil 8'de görülmektedir.

[Şekil 8: Oynanabilir sınıf dizilimi — 10 sınıfın idle_south sprite'ları | figures_v2/class_lineup_sheet.png]
...
Karakter ve silah sprite'ları ayrı üretilir ve çalışma zamanında birleştirilir. `HandAnchorAttach`, `WeaponDatabaseSO`'daki `anchorOffset` ve `gripOffset` verisini kullanarak silahı karaktere prosedürel olarak bağlar; ön/arka el çevirme (flip) mantığı yön değişiminde silahın doğru tarafta görünmesini sağlar. Yön başına el-yuvası hizalaması demo kapsamı dışında bırakılmış; tam hizalama sonraki geliştirme kapsamı olarak planlanmaktadır. Bu birleştirme sistemi Şekil 9'da gösterilmektedir.

[Şekil 9: Silah mount — prosedürel silah bağlama sistemi; karakter ve silah sprite'ları çalışma zamanında `HandAnchorAttach` ile birleştirilir | fig_weapon_mount.png]
...
Demo'da 12 düşman sprite'ı kullanılır; tümü ortak içe aktarma sözleşmesini paylaşır (64×64, PPU 64, Point, sıkıştırma kapalı), chibi high-top-down (~65°) stil, 1 piksel dış çizgi (outline), soğuk slate-gri + siyan-Rift paleti. Kadro: fracture_imp, relic_caster, seam_crawler, plate_widow, hollow_arbitter, rift_gound, spire_choirling, shard_walker, penitent_bruiser, riftbound_augur, hollow_hulk (demo boss'u olarak da büyütülmüş kullanılır) ve rift_acolyte. Düşman kadrosunun kontak sayfası Şekil 10'da görülmektedir.

[Şekil 10: Act-1 düşman kadrosu kontak sheet'i — 12 sprite, Hollow Hulk boss olarak işaretli | figures_v2/mob_roster_sheet.png]
...
Altı temel rol tanımlanmış ve farklı araçlarla doldurulmuştur:

| Rol | Kategori | Ne Yapar |
|---|---|---|
| Orkestratör | Büyük dil modeli (koordinasyon) | Görev dağıtımı, sıralama, sentez, nihai karar |
| Kod Üretim Modülü | Büyük dil modeli (uygulayıcı) | Kod yazma, Unity değişiklikleri, mekanik toplu iş |
| Mimari Çapraz-Doğrulama | Büyük dil modeli (danışman) | Tasarım kararları, mimari değerlendirme, risk analizi |
| Kalite Kontrol Ajanı | Büyük dil modeli (denetçi) | Çıktı kalite kontrolü, hata yakalama (yazar ≠ denetçi) |
...
- **Cross-review zorunluluğu:** Yazar ve reviewer aynı ajan olamaz. Örneğin Kod Üretim Modülü'nün yazdığı knockdown sistemi Mimari Çapraz-Doğrulama Ajanı tarafından, Mimari Çapraz-Doğrulama kanalıyla üretilen bir bileşen ise Kod Üretim Modülü tarafından incelenmiştir.
...
### 8.4 Örnek Görev Akışı: Paralel Görev Kuyruğu

Süreç kuralları, çok görevli bir batch akışında somutlaşır: 11 görev bir karar belgesiyle tanımlanıp birbirine dokunmayan paralel şeritlere ayrılmış, 9'u tamamlanmış, 2'si tanım eksikliği nedeniyle BLOCKED raporlanmıştır. Bu akışta cross-review iki entegrasyon hatasını birleştirme öncesi yakalamış (knockdown i-frame sızıntısı ve çifte resistance), bir ajanın sahne-restore işlemi başka bir değişikliği silince de "tek-Unity-ajan" kuralı bu somut gerekçeyle benimsenmiştir.
...
**Test bağımsızlığı:** "Kodu yazan AI testleri de yazıyorsa bağımsızlık nasıl sağlanır?" sorusuna yanıt şudur: kabul kriterleri, test mantığı ve sınır durumları geliştirici tarafından tanımlanır; ajanlar yalnızca bu insan-tanımlı mantığı koda döker. Görsel QC süreci (Bölüm 9) otomatik testlerin kapsamadığı hataları yakalayan bağımsız bir doğrulama katmanıdır. **İnsan müdahalesi gerektiren alanlar:** tasarım zevki ve oyun hissi hiçbir zaman ajanlara devredilmemiştir; "bu oda sıkıcı mı", "bu knockdown ağır hissediyor mu", "bu güzel mi" kararları yalnızca gerçek oyun oturumundan sonra verilebilmiştir. **Ajan hataları ve korumalar:** sessiz başarısızlık (kota dolan bir uygulayıcı ajanın eski "done" dosyasını tekrar basması) en tehlikeli örüntüdür; bunun üzerine her DONE dosyasının zaman damgasını kontrol etme kuralı getirilmiştir.

Araçlar; orchestrator/reviewer ayrımı, karar belgesi zorunluluğu, tek-Unity-ajan kuralı ve cross-review matrisinden oluşan bir mimariyle orkestre edilmiştir. Projenin katkısı yalnızca AI kullanımında değil, AI çıktısını denetlenebilir mühendislik adımlarına dönüştüren süreç tasarımındadır.

**Çapraz-denetim örneği.** Demo hazırlık aşamasında, bağımsız bir Kalite Kontrol Ajanı danışma sürecine entegre edildi. Bu ajan, diğer danışmanların benimsediği bir öneriyi (koşu içi para birimi göçü) bağımsız kanıtla reddederek demo öncesi kapsam genişlemesinin önüne geçti. Ajanın bulgusu, danışma paneli sentezini değiştirdi ve riskli bir değişiklik uygulanmadan önce elendi. Bu örnek, farklı modellerin çapraz denetimiyle risk tespiti disiplinini somutlaştırmaktadır: çok-danışmanlı süreç yalnızca onay toplamak için değil, gerçek risk tespiti için kullanılmıştır.

### 8.6 İnsan-YZ İş Bölümü ve Çalışma Prensipleri

Proje boyunca farklı rollere sahip yapay zekâ araçlarıyla iş birliği içinde yürütüldü.
...
Tüm koordinasyon, geliştiricinin etkileşim kurduğu terminalde çalışan bir **orkestrasyon katmanı**¹ üzerinden yönetildi. Orkestratörün görevi tek bir büyük işi doğrudan çözmeye çalışmak değildi; işi anlamlı parçalara bölmek, her parçayı uygun uygulayıcıya yönlendirmek, çıktıları bir araya getirmek ve sonuçları geliştirici onayına sunmaktı.

#### Ajan Rolleri

Sistemin üç işlevsel katmanı vardır:

- **Uygulayıcılar** — Kod yazan, dosya değiştiren, doğrulama komutu çalıştıran ajanlardır. Karmaşıklık düzeyine göre iki ayrı Kod Üretim Modülü kullanıldı: çok-dosya değişiklikler için derin bağlam gerektiren model; mekanik ve yinelemeli görevler için daha hafif model¹. Bu modeller arka planda görev kuyruğuna alınarak; biri derin statik analiz, diğeri hızlı bilgi-toplama alanında çalıştırıldı.
- **Denetçiler** — Kalite Kontrol Ajanı, salt-okunur erişimle çalışır; yazan ajandan bağımsız olarak çıktıyı inceler ve doğrulama raporunu doğrudan geliştiriciye iletir. Bu katmanın varlığı, uygulayıcı ile denetçi arasındaki ayrımı kurumsal bir kural olarak hayata geçirir: **yazan denetlemez**.
- **Mimari Değerlendirme Paneli** — Tek bir sistemin ötesine geçen, birden fazla bileşeni aynı anda etkileyen kararlar için üç farklı Mimari Çapraz-Doğrulama Modelinden oluşan bir değerlendirme paneli oluşturuldu. Panel bağımsız bakış açıları üretir; sentezi geliştirici yapar. Bu yöntem, proje boyunca demo kapsam kilitleme, ses mimarisi ve görsel kalite kararları gibi geri dönüşü güç seçimlerde uygulandı.

¹ Projede kullanılan modeller: Orkestrasyon Katmanı ve Kalite Kontrol Ajanı rolünde Claude (Anthropic); Kod Üretim Modülü kanalında Codex (OpenAI) ve Claude alt-ajanları; Mimari Çapraz-Doğrulama Modelleri olarak Gemini Pro ve Gemini Flash (Google DeepMind). Model seçimi proje geliştirme döneminin mevcut araçlarını yansıtmaktadır.

#### Temel Disiplinler

Sistemin işleyebilmesi için aşağıdaki disiplinler proje kuralı olarak belgelendi ve her oturumda uygulandı:

- **İddia ≠ kanıt:** Bir ajanın "başarılı" demesi, işin tamamlandığının kanıtı sayılmadı. Her kritik çıktı veri tabanlı doğrulama (birim testi, ekran görüntüsü, runtime API çağrısı) ile teyit edildi. Bu prensibin somutlaştığı örnek 11.4 no'lu vakada ayrıntılı olarak ele alınmıştır.
- **Tek Unity ajanı:** Aynı anda sahnede değişiklik yapan birden fazla ajan çalıştırılmadı. Bu kural, eş zamanlı sahne değişikliklerinin birbirini sildiği geri alma çakışması vakasından sonra benimsendi (bkz. §11.5).
- **Oturumlararası hafıza:** Yapay zekâ araçları her oturumda geçmiş bağlamı yeniden taramak zorunda kalmadı; tasarım kararları, teknik tercihler ve geçmiş hatalar kalıcı bellek dosyaları ve NotebookLM bilgi tabanı aracılığıyla oturumdan oturuma taşındı. Kod tabanının kendisi ise graphify aracıyla 6925 düğümlü bir bilgi grafiğine dönüştürülerek mimari sorularda ajanların kaynak tüketimini en aza indiren, sorgulanabilir bir hafıza katmanı oluşturuldu.
...
RIMA'nın kalite güvence (QC) süreci üç katmandan oluşur: otomatik birim/entegrasyon testleri, sözleşme (contract) testleri ve sistematik görsel kalite güvencesi. AI'nin kod ürettiği bir bağlamda bu süreç kritiktir: otomatik üretilen bir kod parçası derleniyor olabilir ama sistemin geri kalanıyla beklendiği gibi konuşup konuşmadığı ayrıca doğrulanmalıdır.
...
Kök neden analizi iki ayrı hata ortaya çıkardı. Birincisi 6 şablonu aynı şekilde etkileyen sistematik bir prop hatasıydı: Bridson-Poisson geçişi, sınır hücreleri için koordinatları zemin hücre kümesinin dışına taşıyor; bu geçersiz koordinatlar şablona bake edilince `IsoRoomBuilder` prop'ları ada dışında konumlandırıyordu. İkincisi `combatlarge_twin_basins_01`'de iki havuzu birleştiren tek-hücrelik boşluğun uçurum çözücüyü yanıltması, sprite'ların zeminin ortasından geçmesiydi. Düzeltmede prop koordinatları bake öncesi zemin kümesine karşı doğrulanmaya başlanmış, etkilenen 6 şablon yeniden tohumlanmış, uçurum boşluğu kapatılmıştır.

Yeniden doğrulama kritik ölçütü `propOutsideFloor` sayacının sıfır olmasıydı. Aşağıdaki tablo, düzeltme sonrası üç kritik şablonun doğrulama özetini vermektedir:

| Şablon | Zemin Hücresi | İnşa Edilen Prop | Ada Dışı Prop | Sonuç |
|---|---|---|---|---|
| combat_large_diamond_01 | 212 | 4 | 0 | PASS |
| combatlarge_twin_basins_01 | 599 | 1 | 0 | PASS |
| Treasure_01 | 42 | 4 | 0 | PASS |
...
### 10.2 God-Node Analizi: Araç Katmanı Tezinin Sayısal Kanıtı

Bir grafikte en yüksek bağlantı derecesine sahip node'lar ("god-node"), sistemin etrafında döndüğü merkezlerdir. RIMA'da en bağlı 10 node analiz edildiğinde belirgin bir topolojik dağılım görülür: bunların **6'sı editör/araç (tooling) sınıfıdır**, yalnızca 4'ü oyun/runtime sınıfıdır. Bu, "RIMA aslında bir geliştirme-ortamıdır, üzerine oturan oyun ise bu ortamın bir tüketicisidir" tezinin doğrudan, sayısal kanıtıdır.

En bağlı düğümler Şekil 11'de görselleştirilmiş olup aşağıdaki tabloda özetlenmiştir:

| Düğüm | Tür | Rol |
|---|---|---|
| DirectorMode (168) | Araç (Tooling) | Çalışma zamanı arayüz fabrikası — en bağlı düğüm |
| InPlayMapPaintOverlay (93) | Araç (Tooling) | Oyun-içi harita boyama katmanı |
| RoomPainterWindow (88) | Araç (Tooling) | Editör oda boyama penceresi |
| ChamberSelectBootstrap (84) | Çalışma zamanı | Karakter seçim önyükleyici |
| LargeDungeonMapPainterBase (78) | Araç (Tooling) | Harita boyama taban sınıfı |
| CharacterSelectScreen (75) | Çalışma zamanı | Karakter seçim ekranı |
| MinimalTilePainter (70) | Araç (Tooling) | Karo boyama aracı |
| RuntimeRoomManager (69) | Çalışma zamanı | Çalışma-zamanı oda yöneticisi |
| BuildPlacementController (66) | Araç (Tooling) | Build Mode prop yerleştirme |
| RoomRunDirector (65) | Çalışma zamanı | Oda yaşam-döngüsü |

[Şekil 11: Graphify — en bağlı 10 node; turuncu = editör araçları (6), mavi = oyun/runtime (4) | fig_graphify_godnodes.png]

### 10.3 Tam Graf Görünümü

Şekil 12, tam kod grafiğinde 118 topluluğun renk-kodlu kümeler hâlinde dağılımını göstermektedir. Bu görselleştirme doğrudan bir mimari karara dayanak oluşturdu: araç katmanı ile oyun-sistemleri katmanının kod grafiğinin ayrı bölgelerinde konumlandığının görülmesi, bu iki katmanın birbirinden bağımsız değiştirilebileceğini doğruladı ve dokümantasyondaki "araç / çalışma zamanı" (tooling / runtime) ayrım çerçevesini (bkz. §2.2) somutlaştırdı. Araç sınıfları (Map Designer, Build Mode, Director Mode, boyama araçları) grafiğin merkezinde yoğun bir küme oluştururken, oyun sistemleri (Skills, Combat, Enemies) çevresel kümeler hâlinde bu merkeze bağlanır. Bu topoloji, Bölüm 1.4'te gözlenen "araçlar kod tabanının beşte biri" bulgusunu bağımsız bir metrikle doğrular.

[Şekil 12: Graphify tam kod grafiği — 6925 node, 118 community | fig_graphify_full.png]

---

## 11 KARŞILAŞILAN ZORLUKLAR VE ÇÖZÜMLER

### 11.1 Grafik API Sürücü Uyumsuzluğu

Geliştirme ortamı yükseltildiğinde Unity 6'nın varsayılan Direct3D 12 arka ucunun yeni donanım sürücüsüyle uyumsuz olduğu tespit edildi; `PlayerSettings`'de grafik API önceliği Direct3D 11'e alınarak sorun giderildi. Çıkarım: geliştirme ortamı değiştiğinde varsayılan platform ayarlarının uygunluğu doğrulanmalıdır.
...
**Çözüm.** Sistem yön-bazlı komşu kontrolüne göre yeniden yazıldı (bkz. §6.2 mantık özeti); SW/SE void hücreleri "inward tuck" vektörüyle yerleştirildi. Çıkarım: görsel hatada yüzeysel yamalardan önce algoritmanın hangi koşulda tetiklendiği belirlenmelidir.
...
**Çözüm.** `Destroy(gameObject)` → `Destroy(this)` ile yalnızca fazla bileşen yok edildi; nesnenin diğer bileşenleri sağlıklı kaldı ve akış düzeldi. Çıkarım: paylaşılan bir GameObject üzerinde singleton uygulanırken hangi kapsamın (bileşen vs. nesne) hedeflendiği açıkça belirlenmelidir.

### 11.4 Combat-Okunabilirlik Vakası: "Yeşil-Assert ≠ Çalışıyor"

Bu vaka, otomatik testlerin geçmesinin tek başına çalışan bir sistemi garanti etmediğini gösteren bir örnektir.
...
**Kök neden (veri-güdümlü).** Tek-arena doğrulamasıyla sorun izole edildi: `BaseMobBehavior.detectionRange = 8`, üretim (spawn) mesafesi (9.12) küçük → düşmanlar kalıcı boşta (Idle) durumunda kalıyordu (HalfThrall prefab'da değer 6). Kaynak yönetimi sağlamdı; sorun yalnızca menzildeydi.
...
**Çözüm.** Her gönderim sonrası "done" dosyasının zaman damgası görev başlangıcıyla karşılaştırıldı; sahne içeren işler öncesi değişiklikler commit'lendi; tek-Unity-ajan kuralı benimsendi. Çıkarım: AI-ajan pipeline'ında güven sözlü teminata değil doğrulama mimarisine dayanmalıdır.

### 11.6 Yetenek Aktivasyonunda "Harca-Sonra-Veto" Hatası

**Sorun.** Bazı yetenekler hedef menzil dışında veya koşul sağlanmadan aktivasyon girişiminde bulunulduğunda dahi (no-op) mana ve bekleme süresini tüketiyordu. Bu durum "ölü buton" algısı yaratıyor, oyuncunun kaynağı boşa harcamasına neden oluyordu.

**Teşhis.** Maliyet ve bekleme süresi harcaması `SkillBase` temel sınıfında yetenek mantığı çalışmadan önce gerçekleşiyordu; menzil ya da hedef kontrolleri ise yetenek gövdesinde geç devreye giriyordu.

**Çözüm.** Temel sınıfa (`SkillBase`) maliyet ve bekleme süresi harcanmadan önce çalışan bir `CanExecute()` veto kancası eklendi; menzil-kapılı yetenekler bu kancayı uygulayarak boşa harcamayı önler. Ek olarak yetersiz-kaynak ve veto durumlarında kısa ses, atıcı parlaması ve bildirim eklendi; bekleme süresindeki tuş tekrarları ise kasıtlı olarak sessiz bırakıldı (commit `6ba61ff5`, `d0e6466e`). Çıkarım: eylem maliyetini kapsayan veto mantığı, yetenek uygulamasında değil temel sınıfta yer almalıdır.

### 11.7 Bileşenler Arası Mantıksal-Bağlantı Hatası

**Sorun.** Koşu yönlendiricisi (`RoomRunDirector`) oda-temizleme olayını ilerleme istatistiklerine (`RunStats`) iletmiyordu; temizlenen oda sayısı her koşuda sıfır kalıyor, zafer ekranı ve Echo ödülü hatalı değer gösteriyordu. Bu, bileşen bağlantısındaki bir mantıksal-bağlantı hatasıydı; birim testleri yeşil geçtiği hâlde tam uçtan uca akış kırıktı.

**Çözüm.** `RoomRunDirector` ile `RunStats` arasına oda-temizleme köprüsü eklendi; her "Cleared" geçişi artık ilerleme sayacına doğru biçimde yansıtılıyor. Bu hata, tam-akış entegrasyon testiyle yakalandı.

Çıkarım: birim testleri bileşen mantığını doğrular; sistem sınırlarındaki mantıksal-bağlantı hatalarının tespitinde uçtan uca entegrasyon testleri tamamlayıcı bir doğrulama katmanıdır.

### 11.8 Boss Faz Bütünlüğü: Faz Atlama

**Sorun.** Penitent Sovereign boss karşılaşmasında anlık yüksek hasar (burst) Faz-2'yi atlayarak doğrudan Faz-3'e geçişe neden olabiliyordu. Bu durum, Faz-2 mekaniğinin oyuncuya hiç gösterilmemesi demekti ve kasıtlı zorluğun devre dışı kalmasına yol açıyordu.

**Çözüm.** Faz-2'ye giriş zamanı kaydedildi ve Faz-3 tetikleyicisi en az 8 saniyelik bir faz-kilidiyle kapatıldı; böylece yüksek anlık hasarda bile Faz-2 mekaniği her zaman oynanır hâle geldi (commit `6ba61ff5`). (Not: boss saldırı telegraph sistemi P1/P2/P3 fazlarının tamamında zaten aktif olup doğrulanmıştır — bkz. §4.4.) Çıkarım: faz-geçiş eşiklerini yalnızca can değerine bağlamak yeterli değildir; anlık hasar piklerini sönümleyecek bir zaman kilidi gereklidir.

### 11.9 Paylaşılan Çalışma Zamanı Değişkenine Çift Sahip Yazma

**Sorun.** `HitStop` ve `HitPauseDriver` bileşenlerinin her ikisi de `Time.timeScale`'i eş zamanlı düzenliyordu; çakışan yazımlar oyunu uzun süreli donma durumuna sokuyordu. Ek olarak kare hızı üst sınırsız bırakıldığından bazı ortamlarda performans tıkanması gözlemlendi.

**Çözüm.** `timeScale` yazma yetkisi tek bir sürücüye (`HitPauseDriver`) devredildi; eski `HitStop` sürücüsü `[Obsolete]` olarak işaretlenip temel saldırı yolundan (`MarkPulseBehavior`) çıkarıldı; böylece duraklatma sırasında `previousTimeScale=0` yakalanıp 0'a "geri yüklenen" çift-sahip (dual-owner) donması ortadan kalktı. Kare hızı için `FrameRateGuard` bileşeniyle 60 FPS üst sınırı tanımlandı. Her iki düzeltme editörde doğrulandı.

Çıkarım: paylaşılan çalışma zamanı değişkenlerine (örn. `timeScale`) tek-sahip yazma disiplini uygulanması, özellikle birden fazla efekt sürücüsünün bir arada bulunduğu sistemlerde doğruluk ve öngörülebilirlik açısından kritiktir.

### 11.10 Tekil Yönetici Yaşam-Döngüsü Hatası: Statik Bayrağın Sıfırlanmaması

**Sorun.** Oyun-içi yeniden başlatma sonrasında açılış yetenek seçimi (draft) ekranı açılmıyordu.

**Teşhis.** `DraftManager` tekil yöneticisinin (singleton) statik `_shuttingDown` bayrağı, sahne yeniden yüklemesinde sıfırlanmadan açık kalıyordu. Bu bayrak yalnızca oyun modu girişinde temizlendiğinden, sahne yeniden yüklemesiyle oluşan yeni örnekte `Instance` erişimcisi null döndürüyor ve açılış draft'ı tetiklenemiyordu.

**Çözüm.** `OnDestroy` artık bu bayrağı set etmiyor; bayrak yalnızca gerçek uygulama kapanışında ayarlanıyor. Bu düzeltme, projede aynı hata sınıfı için önceden yerleştirilmiş kanonik deseni (`AttackTokenManager`) genelleştirir. Aynı gizli hata üç kardeş tekil yöneticide (`RunStats`, `CheckpointManager`, `BuildTileBrushController`) de giderildi. Düzeltme çalışma zamanında doğrulandı: yeniden başlatma → açılış draft ekranı açılıyor. Bağımsız bir denetim ajanı düzeltmeyi onayladı.

Çıkarım: çapraz-kesen bir yaşam-döngüsü hatası, tek bir referans desenin tutarlı yayılmasıyla giderilebilir; singleton uygulamalarında sahne yeniden yüklemesi ile gerçek uygulama kapanışı arasındaki ayrım açıkça modellenmeli ve test edilmelidir.

### 11.11 Savunma Amaçlı Koşul: Zaman-Ölçeği ile Yetenek-Duraklatması Etkileşimi

**Sorun.** Açık bir yetenek seçimi (draft) ekranı sırasında `timeScale`'i geri yükleyen kod yolu, draft duraklatmasını geçersiz kılarak oyunun duraklamadan çıkmasına neden olabiliyordu.

**Teşhis.** Sorun, §11.9'da ele alınan çift-sahip yazma örüntüsünün bir türeviydi: `timeScale` tek sahibi `HitPauseDriver`'a devredildikten sonra bile, yetenek-duraklatma akışında `timeScale`'e dokunan geride kalmış bir yol mevcuttu. Bu yol temiz akışta tetiklenmediğinden yalıtılmış testlerde görünmüyor, yalnızca birden fazla efekt sürücüsünün aynı anda etkin olduğu durumda ortaya çıkıyordu.

**Çözüm.** Yetenek-duraklatma durumunu kontrol eden bir savunma amaçlı koşul eklendi; draft aktifken `timeScale` geri yüklemesi engellendi.

Çıkarım: §11.9 ve §11.11 birlikte değerlendirildiğinde, `timeScale` yönetimini kararlı kılmak için tek-sahip yazma disiplininin yanı sıra tüm geri yükleme yollarında durum-farkındalığının zorunlu olduğu görülmektedir.
...
Projenin başındaki soru — tek geliştirici kapsamlı bir aksiyon-roguelite'i nasıl yapabilir? — iki paralel yanıtla ele alındı: içerik ile sistemi ayıran veri-güdümlü mimari ve bunun üzerinde çalışan çok-ajanlı geliştirme süreci. Bölüm 9 ve 10'da ayrıntılı sunulan sayısal kanıtlara ek olarak, ulaşılan temel çıktılar şöyle özetlenebilir:

- **Oyun sistemi altyapısı:** 10 sınıflık veri altyapısı (her biri farklı kaynak ekonomisi ve oynanış diliyle); 4 sınıfın çalışan kontrolcüsü mevcut, demo'da Warblade ve Elementalist uçtan uca oynanabilir; tam oynanabilir akış (Ana Menü → Karakter Seçimi → Savaş → Yetenek Seçimi → Boss → Meta-ilerleme).
- **Araç ve içerik katmanı:** Oyun-içi araç zinciri (Build Mode, Director Mode, F1, Map Designer, Room Browser, RoomJsonImporter, ScreenshotMode); oyun-hissi katmanı (darbe-duraklaması, CombatJuice, telegraph, 18 CC0 SFX + 9 kanal).
- **Graphify mimari kanıtı:** En bağlı 10 düğümün 6'sı araç sınıfı; tooling ve runtime katmanlarının grafikte ayrı bölgelerde konumlandığı görsel olarak doğrulandı (bkz. §10).
...
- **Silah üretimi ve sınıf derinleştirme:** Kalan sınıfların sınıfa özel silahları (sprite + mekanik) ve placeholder skill'lerin gerçek implementasyonu en büyük içerik bloğudur.
- **Elementalist büyü VFX:** Particle/shader düzeyinde efekt üretimi; kod ve araç altyapısı hazır, kullanıcı onayıyla başlayacak. Elementalist'in 8 ayrı yön idle klibi üretilmiş ve animasyon denetleyicisine entegre edilmiştir; sınıf gerçek 8-yön ile oynanabilir durumdadır. Kalan kapsam: büyü particle/shader VFX efektleri.
...
## EK A — DEMO OYUN AKIŞI: EKRAN GÖRÜNTÜLERİ

Bu ek, RIMA demo koşusunun doğrusal akışını playtest ekran görüntüleriyle belgeler. Görüntüler 2026-06-19 tarihli playtest oturumundan alınmıştır.

### A.1 Savaş ve Yetenek Seçimi

Koşu, Ana Menü → Attunement Chamber (karakter seçimi) → _Arena (aktif koşu) akışını izler. Aşağıdaki görüntü, Warblade sınıfıyla ilk kart seçiminin tamamlanmasının ardından savaş odasının aktif durumunu göstermektedir.

[Şekil 13: Warblade — ilk kart seçimi (card0) sonrası aktif savaş durumu; skill bar sol-alt köşede güncel | test1_warblade_card0_post_pick.png]

### A.2 Elementalist LMB — Ateş Topu Uçuşu ve Çarpma

Elementalist sınıfının sol tuş (LMB) temel saldırısı ölçülü bir ateş topu fırlatır. Aşağıdaki iki görüntü sırasıyla atış sırasındaki uçuş fazını ve hedefe çarptığındaki patlama efektini göstermektedir.

[Şekil 14: Elementalist LMB ateş topu — uçuş fazı; bloom kontrollü, sprite ölçüsü kalibre edilmiş | lmb_measured_final.png]

[Şekil 15: LMB ateş topu çarpma patlaması — çarpma anı VFX; kısa süreliğine genişleyen daire efekti | lmb_impact_explosion.png]

### A.3 Düşman Durum Boyaması: Yanma ve Üşüme

Elementalist'in Ateş ve Buz yetenekleri düşmana durum renk boyası uygular; bu geri bildirim oyuncuya düşmanın hangi durumda olduğunu anında iletir.

[Şekil 16: Düşman yanma durumu (Burn) — kırmızı renk boyası; Elementalist ateş saldırısı sonrası | ses_enemy_burn_red.png]

[Şekil 17: Düşman üşüme durumu (Chill) — mavi renk boyası; Elementalist buz saldırısı sonrası | ses_enemy_chill_blue.png]

### A.4 Oyun-İçi Araçlar: Build Mode ve Map Designer

Playtest, oyunculara açık geliştirici araçlarını da kapsamaktadır. Build Mode (F2) prop yerleştirmeyi destekler; Map Designer'ın uçurum üreteci prosedürel olarak yüzen ada odaları oluşturur.

[Şekil 18: Build Mode — prop yerleştirilmiş oyun-içi seviye editörü görünümü | part2_buildmode_prop_placed.png]

[Şekil 19: Map Designer uçurum üreteci — prosedürel cliff-island oda çıktısı | part1_cliff_generate_result.png]
