...
Bu odalar, harici araçlarla üretilen JSON şablonlardan `RoomJsonImporter` aracılığıyla içe aktarılır; her dosya bir oda kimliği, ızgara boyutu ve zemin bölgelerini taşır. Aşağıdaki kısaltılmış excerpt (gerçek `act1_entry_hall.json` dosyasından) bu oda formatını gösterir:

```
{
  "schema_version": "1.0",
  "room_id": "act1_entry_hall",
  "room_type": "spawn",
  "width": 32,
  "height": 24,
  "floor": {
    ...
    "zones": [
      { "material": "walkway",
        "rect": { "x": 13, "y": 2, "width": 6, "height": 20 },
        "blend_edges": true }
    ]
  },
  "walls": [
    { "prefab": "archway_n", "x": 15, "y": 22, "rotation": 0 }
  ]
}
```

Tekil oda dosyalarının yanında bir `_manifest.json`, odaların birbirine nasıl bağlandığını tanımlayan bir `door_graph` taşır; bu graf doğrudan run-map'in dallanmasını besler. Aşağıdaki excerpt, giriş odasının (entry_hall) Kuzey/Doğu/Batı dallarıyla altı-odalı bağlılık grafiğini gösterir:

```
"door_graph": {
  "act1_entry_hall": {
    "south": null,
    "north": "act1_north_antechamber",
    "east":  "act1_east_corridor",
    "west":  "act1_west_chamber"
  },
  "act1_west_chamber":     { "east": "act1_entry_hall", "north": "act1_north_antechamber" },
  "act1_east_corridor":    { "west": "act1_entry_hall", "east": "act1_treasure_vault" },
  "act1_treasure_vault":   { "west": "act1_east_corridor" },
  "act1_north_antechamber":{ "south": "act1_entry_hall", "north": "act1_shattered_throne" },
  "act1_shattered_throne": { "south": "act1_north_antechamber" }
}
```
...
Karakter ve silah sprite'ları ayrı üretilir ve çalışma zamanında birleştirilir. `HandAnchorAttach`, `WeaponDatabaseSO`'daki `anchorOffset` ve `gripOffset` verisini kullanarak silahı karaktere prosedürel olarak bağlar; ön/arka el flip mantığı yön değişiminde silahın doğru tarafta görünmesini sağlar. Yön başına el-yuvası hizalaması (per-direction hand-socket) demo kapsamı dışında bırakılmış; tam hizalama post-demo çalışması olarak planlanmaktadır.
...
**Çözüm.** Sistem yön-bazlı komşu kontrolüne göre yeniden yazıldı (bkz. §6.2 mantık özeti); SW/SE void hücreleri "inward tuck" vektörüyle yerleştirildi. Ders: görsel hatada yüzeysel yamalardan önce algoritmanın hangi koşulda tetiklendiği belirlenmelidir.
...
- **Tam oynanabilir döngü** — MainMenu → Attunement Chamber → Combat → Skill Draft → Boss → (ikincil sınıf / unlock draft) → post-boss Combat → Zafer/Ölüm → Meta-ilerleme.
