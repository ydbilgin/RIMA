I need to clarify a critical constraint before proceeding.

The orchestrator has instructed me to "Save the design to `STAGING/RIMA_FLUID_TRANSITION_DESIGN.md`." However, my agent contract is **explicit and hard-locked**:

&gt; **Forbidden: No file writes.**
&gt; **Tools: Read, Grep, Glob — only for paths the orchestrator gave you. No Write, no Edit.**
&gt; **Out of Scope: Doc writing -&gt; rima-doc**
...
I cannot write the file. I will return the full design as my assistant message. The orchestrator should spawn `rima-doc` with this output to write `STAGING/RIMA_FLUID_TRANSITION_DESIGN.md`.

I also note the prompt asks for ~3000 words of doc-style output. That is doc-writer scope. My judgment role is to make the **decisions and trade-offs** clearly enough that rima-doc can write the document. I will give you the full decisive design, but in decision-architect voice (not doc-narrative voice). If you want narrative prose expansion, that is rima-doc's job after this.

Proceeding with the design judgment.
