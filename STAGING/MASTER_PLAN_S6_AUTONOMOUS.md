...
- 🔨 **Batch G** (cx-yekta `bq82fgqku`, in flight): RunStats singleton (kills/time/rooms/build) + Build-Seed + Victory/Death CTA'ya gerçek veri + Copy-Build-Seed clipboard. Pure .cs.
- ⏭️ NEXT (otonom, image_gen unlocked): Batch H image-gen Priority-A (menu/death/victory bg + wishlist btn + logo + silhouette → Resources/UI/RIMA/) → Batch I screen-image wiring (.cs load-if-present fallback procedural) → Batch J Priority-B/C + skill icons. **SCENE-rig/weapon-prefab/play-verify = GATED** (`SCENE_WIRING_RUNBOOK_S6.md`, MCP-restart/kullanıcı).
