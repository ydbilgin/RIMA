...
- **Sprite canvas:** "128x128" (ara rapor) → **64px (GÜNCEL — user 2026-06-01: "karakterim değişti, 64px'e döndü")**. Karakter YENİDEN tasarlandı + canvas 64px. Sprite spec/import ayarları + karakter görsel kimliği bölümü buna göre güncellenecek. (Ara-dönem 120px artık geçmiş.)
...
## Önerilen yapı (~30-40 sayfa)
1. Kapak / İçindekiler / Özet (TR) + Abstract (EN)
2. Giriş — tür, problem tanımı, hedef (genişlet)
3. Literatür & referans oyun analizi — her oyun ayrı alt bölüm + RIMA'ya aktarım gerekçesi (genişlet)
4. Oyun tasarımı — döngü, 10 sınıf, dual-class kırılma, tag/sinerji, yetenek kademe (Common→Legendary) modeli (örneklerle genişlet)
5. Sanat yönü & karakter pipeline — PixelLab, 8-dir, run-first anim, stil tutarlılığı
6. **Çevre/harita üretim pipeline + iso yön kararı + Map Designer aracı** (YENİ, büyük bölüm)
7. Teknik mimari — Unity 6/URP 2D, C# script mimarisi (sınıf diyagramı), Pixel-Perfect kamera 640x360, depth-sort custom axis
8. **Agentic AI destekli geliştirme metodolojisi** (YENİ, genişlet)
9. Test & doğrulama — EditMode/PlayMode, script validation
10. Karşılaşılan zorluklar & çözümler (genişlet — iso floor kök-neden, MCP/dialog-storm dersleri vb.)
11. Yol haritası / sonraki adımlar
12. Sonuç
13. Kaynakça + Ekler (screenshots, tablolar)
