...
{
    public sealed class ScatterAlongStrokeDataExecutor : IBrushExecutor
    {
        ...
        public BrushExecutorResult Apply(BrushStroke stroke, BrushLayerOperation op)
        {
            ...
            {
                return new BrushExecutorResult { success = false, errorMessage = "Scatter data operation or AssetPool is null" };
            }

            if (op.roomDecalData == null)
            {
                return new BrushExecutorResult { success = false, errorMessage = "RoomDecalDataSO is null" };
            }
            ...
            Undo.SetCurrentGroupName("Brush Scatter Data Stroke");
            Undo.RecordObject(op.roomDecalData, "Brush Scatter Data Stroke");
            ...
            try
            {
                for (int i = 0; i < sampleCount; i++)
                {
                    ...
                    if (!RoomDecalDataExecutorUtility.TryCreatePlacement(candidate, op, i, out placement))
                    {
                        continue;
                    }
            ...
            return RoomDecalDataExecutorUtility.BuildResult(op.roomDecalData, count);
        }
