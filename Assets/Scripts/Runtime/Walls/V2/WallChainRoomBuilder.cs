...
namespace RIMA.Walls.V2
{
    ...
    public class WallChainRoomBuilder : MonoBehaviour
    {
        ...
        public bool preferRealAssets = true;

        [System.NonSerialized] private RoomSpec lastSpec;
        [System.NonSerialized] private HashSet<Vector2Int> lastFootprint;
        ...
        public void Build(RoomSpec spec)
        {
            ...
            lastSpec = spec;
            lastFootprint = new HashSet<Vector2Int>(footprint);
        ...
        List<EdgeSegment> ExtractRearEdges(HashSet<Vector2Int> footprint)
        {
            ...
            // Sort key: (y, x) keeps same-row rear segments adjacent before GroupConsecutive scans x.
            list.Sort((a, b) =>
            {
        ...
        List<EdgeSegment> ExtractSideEdges(HashSet<Vector2Int> footprint, int side)
        {
            ...
            // Sort key: (x, y) keeps same-column side segments adjacent before GroupConsecutive scans y.
            list.Sort((a, b) =>
            {
        ...
        List<EdgeSegment> ExtractFrontEdges(HashSet<Vector2Int> footprint)
        {
            ...
            foreach (var c in footprint)
                if (!footprint.Contains(new Vector2Int(c.x, c.y - 1)))
                    list.Add(new EdgeSegment { cell = c, dir = new Vector2Int(0, -1) });
            // Sort key: (y, x) keeps same-row front segments adjacent before GroupConsecutive scans x.
            list.Sort((a, b) =>
            {
                int yc = a.cell.y.CompareTo(b.cell.y);
                return yc != 0 ? yc : a.cell.x.CompareTo(b.cell.x);
        ...
        void EnforceDoorCenter(List<EdgeSegment> rear, RoomSpec spec)
        {
            ...
            if (registry.GetByType(WallPieceType.DoorArch, WallDirection.Rear, preferRealAssets) == null)
        ...
        void FillRunWithSpans(int start, int end, int fixedCoord, WallDirection dir, RoomSpec spec, bool horizontal, bool startIsCorner, bool endIsCorner)
        {
            ...
            if (length == 1)
            {
                SpawnConnectorIfCorner(start, fixedCoord, horizontal, startIsCorner || endIsCorner);
                bool flipX;
            ...
            if (length == 2)
            {
                SpawnConnectorIfCorner(start, fixedCoord, horizontal, startIsCorner);
                SpawnConnectorIfCorner(end, fixedCoord, horizontal, endIsCorner);
                bool flipX;
                var single = GetSpanForLength(dir, 1, out flipX);
                if (single != null)
                {
            ...
            while (spanLength > 0)
            {
                ...
                {
                    SpawnDoorEndConnectors(cursor, fixedCoord, start, end, horizontal);
        ...
        void SpawnConnectorIfCorner(int coord, int fixedCoord, bool horizontal, bool isCorner)
        {
            if (!isCorner) return;
            SpawnPiece(WallPieceType.Connector, WallDirection.Any, GetCellWorld(coord, fixedCoord, horizontal));
        }

        void SpawnDoorEndConnectors(int doorStart, int fixedCoord, int runStart, int runEnd, bool horizontal)
        {
            int left = doorStart - 1;
            int right = doorStart + 2;
            if (left >= runStart)
                SpawnPiece(WallPieceType.Connector, WallDirection.Any, GetCellWorld(left, fixedCoord, horizontal));
            if (right <= runEnd)
                SpawnPiece(WallPieceType.Connector, WallDirection.Any, GetCellWorld(right, fixedCoord, horizontal));
        }
        ...
        void PlaceCenterReserved(RoomSpec spec)
        {
            ...
            go.transform.localPosition = GetIsoWorld(spec.widthCells * 0.5f, spec.heightCells * 0.5f, -0.05f);
        ...
        void PlaceWaterPoolZones(RoomSpec spec)
        {
            ...
            for (int i = 0; i < spec.waterPoolRects.Count; i++)
            {
                ...
                go.transform.localPosition = GetIsoWorld(rect.x + rect.width * 0.5f, rect.y + rect.height * 0.5f, 0.05f);
        ...
        {
            float x = horizontal ? a : b;
            float y = horizontal ? b : a;
            return GetIsoWorld(x, y, 0f);
        }

        Vector3 GetIsoWorld(float x, float y, float z)
        {
        ...
        {
            flipX = false;
            if (preferRealAssets && type == WallPieceType.SideWall && dir == WallDirection.SideLeft)
            {
                var mirroredReal = registry.GetPiece(type, WallDirection.SideRight, 0, true);
                if (IsRealData(mirroredReal))
                {
                    flipX = true;
                    return mirroredReal;
                }
            }
            var data = registry.GetByType(type, dir, preferRealAssets);
            if (data == null && dir == WallDirection.SideRight)
            {
                data = registry.GetByType(type, WallDirection.SideLeft, preferRealAssets);
                flipX = data != null;
            }
            if (data == null && dir == WallDirection.SideLeft)
            {
                data = registry.GetByType(type, WallDirection.SideRight, preferRealAssets);
                flipX = data != null;
            }
            return data;
        }
        ...
        {
            flipX = false;
            if (preferRealAssets && dir == WallDirection.SideLeft)
            {
                var mirroredReal = registry.GetSpanForLength(WallDirection.SideRight, length, true);
                if (IsRealData(mirroredReal))
                {
                    flipX = true;
                    return mirroredReal;
            ...
            var data = registry.GetSpanForLength(dir, length, preferRealAssets);
            if (data == null && dir == WallDirection.SideRight)
            {
                data = registry.GetSpanForLength(WallDirection.SideLeft, length, preferRealAssets);
                flipX = data != null;
            }
            if (data == null && dir == WallDirection.SideLeft)
            {
                data = registry.GetSpanForLength(WallDirection.SideRight, length, preferRealAssets);
                flipX = data != null;
            }
            return data;
        }

        static bool IsRealData(WallPieceData data)
        {
            return data != null && !string.IsNullOrEmpty(data.id) && data.id.EndsWith("_real", System.StringComparison.Ordinal);
        }
        ...
        void OnDrawGizmos()
        {
            ...
            DrawCachedFootprintGizmos();

            foreach (Transform t in roomParent)
                DrawSpawnedPieceGizmo(t);
        }

        void DrawCachedFootprintGizmos()
        {
            if (lastSpec == null || lastFootprint == null) return;

            int minX = 0;
            int minY = 0;
            int maxX = Mathf.Max(0, lastSpec.widthCells - 1);
            int maxY = Mathf.Max(0, lastSpec.heightCells - 1);
            foreach (var cell in lastFootprint)
            {
            ...
            Vector3 cellSizeGizmo = new Vector3(0.9f * cellSize, 0.9f * cellSize, 0.01f);
            for (int x = minX; x <= maxX; x++)
            {
                for (int y = minY; y <= maxY; y++)
                {
                    bool walkable = lastFootprint.Contains(new Vector2Int(x, y));
                    Gizmos.color = walkable ? Color.green : Color.red;
                    Gizmos.DrawWireCube(roomParent.TransformPoint(GetCellWorld(x, y, horizontal: true)), cellSizeGizmo);
                }
            }
        }

        void DrawSpawnedPieceGizmo(Transform t)
        {
            var piece = t.GetComponent<WallPiece>();
            if (piece == null || piece.data == null)
            {
                if (t.name.Contains("SocketMarker"))
                {
            ...
            Gizmos.color = GetPieceGizmoColor(piece.data.type);
            Gizmos.DrawWireCube(t.position, new Vector3(0.65f * cellSize, 0.65f * cellSize, 0.01f));

            Gizmos.color = Color.blue;
            DrawSocketGizmo(piece.leftSocket);
            DrawSocketGizmo(piece.rightSocket);
            DrawSocketGizmo(piece.seamSocketLeft);
            DrawSocketGizmo(piece.seamSocketRight);
        }

        Color GetPieceGizmoColor(WallPieceType type)
        {
            switch (type)
            {
                ...
                case WallPieceType.DoorArch:
                    return new Color(0.65f, 0.2f, 1f);
        ...
        void DrawSocketGizmo(Transform socket)
        {
            if (socket == null) return;
            Gizmos.DrawSphere(socket.position, 0.1f);
        }
    }
}
