...
//   7. serializes the in-memory RoomDataDTO → StreamingAssets/live/room_current.json
...
using RIMA.Live;                        // RuntimeAssetRegistry, RegistryEntry, legacy live-tool adapter model
...
namespace RIMA.LiveTool
{
    ...
    public sealed class ToolBootstrap : MonoBehaviour
    {
        ...
        private RoomDataDTO _liveDoc;

        // Temporary adapter for existing live-tool router/handle helpers that still mutate RoomLayoutData.
        private RoomLayoutData _routerDoc;
        ...
        private void Start()
        {
            ...
            _routerDoc = ToRoomLayoutData(_liveDoc);
            ...
            Debug.Log($"[ToolBootstrap] Ready — {_registry.Count} registry entries, doc '{_liveDoc.roomId}'.");
        }
        ...
        /// <summary>
        /// Serialize the current in-memory RoomDataDTO → room_current.json using the
        ...
        /// field names/format match RoomDataDTO exactly.
        /// </summary>
        public void RequestSave()
        {
            ...
            if (_routerDoc != null)
                _liveDoc = ToRoomDataDto(_routerDoc, _liveDoc);

            EnsureDtoDefaults(_liveDoc);
            ...
            try
            {
                ...
                // 2) write JSON (RoomDataDTO fields match the game live reloader).
        ...
        private void DoPaint(Vector2 panelPointerPos)
        {
            ...
            // BrushExecutorRouter still mutates a legacy adapter document; RequestSave writes RoomDataDTO.
            BrushExecutorRouter.PaintResult result =
                _router.Paint(CurrentMode, entry, cell, world, CurrentRotation, _routerDoc);
            if (result.changed)
                SyncLiveDocFromRouter();
        ...
        {
            (Vector3Int cell, Vector3 world) = ScreenCellUnderPointer(panelPointerPos);
            BrushExecutorRouter.PaintResult result = _router.Erase(CurrentMode, cell, world, _routerDoc);
            if (result.changed)
                SyncLiveDocFromRouter();
        }
        ...
        private void Update()
        {
            ...
            // write collider overrides into the legacy adapter doc when dragged.
            if (_colliderHandles != null && _selectedInstance != null)
                _colliderHandles.Tick(_routerDoc);   // C7 API — mutates _routerDoc.collider_overrides
        ...
        private RoomDataDTO LoadOrCreateLiveDoc()
        {
            if (File.Exists(JsonPath))
            {
                try
                {
                    ...
                    RoomDataDTO parsed = JsonUtility.FromJson<RoomDataDTO>(json);
                    if (parsed != null)
                    {
                        bool hasDtoContent =
                            !string.IsNullOrEmpty(parsed.roomId) ||
                            (parsed.floorCells != null && parsed.floorCells.Count > 0) ||
                            (parsed.cliffCells != null && parsed.cliffCells.Count > 0) ||
                            (parsed.propPlacements != null && parsed.propPlacements.Count > 0);
                        if (hasDtoContent)
                        {
                            EnsureDtoDefaults(parsed);
                            return parsed;
                        }
                    }

                    RoomLayoutData legacy = RoomLayoutData.FromJson(json);
                    if (legacy != null)
                    {
                        parsed = ToRoomDataDto(legacy, null);
                        EnsureDtoDefaults(parsed);
                        return parsed;
                    }
                }
            ...
            EnsureDtoDefaults(dto);
            return dto;
        }

        private void SyncLiveDocFromRouter()
        {
            if (_routerDoc == null) return;
            _liveDoc = ToRoomDataDto(_routerDoc, _liveDoc);
        }

        private static void EnsureDtoDefaults(RoomDataDTO dto)
        {
            if (dto == null) return;
            if (string.IsNullOrEmpty(dto.roomId)) dto.roomId = "tool_room";
            if (string.IsNullOrEmpty(dto.displayName)) dto.displayName = "Tool Room";
            if (dto.floorCells == null) dto.floorCells = new List<RoomData.TileCellRecord>();
            if (dto.cliffCells == null) dto.cliffCells = new List<RoomData.TileCellRecord>();
            if (dto.wallCells == null) dto.wallCells = new List<WallCell>();
            if (dto.propPlacements == null) dto.propPlacements = new List<RoomData.PropPlacement>();
            if (dto.portalPlacements == null) dto.portalPlacements = new List<RoomData.PortalPlacement>();
        }

        private static RoomLayoutData ToRoomLayoutData(RoomDataDTO dto)
        {
            if (dto == null)
                dto = new RoomDataDTO();
            EnsureDtoDefaults(dto);
            RoomLayoutData data = new RoomLayoutData
            {
                ...
                floor_tiles = new List<FloorTileData>(),
                cliff_cells = new List<CliffCellData>(),
                prop_instances = new List<PropData>(),
                collider_overrides = new List<ColliderOverrideData>(),
            };

            foreach (RoomData.TileCellRecord cell in dto.floorCells)
            {
                if (string.IsNullOrEmpty(cell.assetGuidOrName)) continue;
                data.floor_tiles.Add(new FloorTileData
                {
                    cell = new[] { cell.cell.x, cell.cell.y, cell.cell.z },
                    tile_guid = cell.assetGuidOrName,
                });
            }

            foreach (RoomData.TileCellRecord cell in dto.cliffCells)
            {
                if (string.IsNullOrEmpty(cell.assetGuidOrName)) continue;
                data.cliff_cells.Add(new CliffCellData
                {
                    cell = new[] { cell.cell.x, cell.cell.y, cell.cell.z },
                    tile_guid = cell.assetGuidOrName,
                    is_decor = false,
            ...
            for (int i = 0; i < dto.propPlacements.Count; i++)
            {
                RoomData.PropPlacement prop = dto.propPlacements[i];
                if (string.IsNullOrEmpty(prop.assetGuidOrName)) continue;
                data.prop_instances.Add(new PropData
                {
                    prefab_guid = prop.assetGuidOrName,
                    position = new[] { prop.position.x, prop.position.y, prop.position.z },
                    rotation = prop.rotation,
                    instance_id = $"{prop.assetGuidOrName}_{i}",
                });
            }
        ...
        private static RoomDataDTO ToRoomDataDto(RoomLayoutData data, RoomDataDTO previous)
        {
            RoomDataDTO dto = previous ?? new RoomDataDTO();
            dto.roomId = string.IsNullOrEmpty(data?.room_id) ? dto.roomId : data.room_id;
            dto.displayName = data?.metadata != null && !string.IsNullOrEmpty(data.metadata.name)
                ? data.metadata.name
                : dto.displayName;
            dto.floorCells = new List<RoomData.TileCellRecord>();
            dto.cliffCells = new List<RoomData.TileCellRecord>();
            dto.propPlacements = new List<RoomData.PropPlacement>();

            if (data?.floor_tiles != null)
            {
                foreach (FloorTileData tile in data.floor_tiles)
                {
                    if (tile == null || tile.cell == null || tile.cell.Length < 3) continue;
                    Vector3Int cell = new Vector3Int(tile.cell[0], tile.cell[1], tile.cell[2]);
                    dto.floorCells.Add(new RoomData.TileCellRecord
                    {
                        assetGuidOrName = tile.tile_guid,
                        cell = cell,
                        worldPos = new Vector3(cell.x, cell.y, cell.z),
                        rotation = 0f,
                        scale = Vector2.one,
                    });
                }
            ...
            if (data?.cliff_cells != null)
            {
                foreach (CliffCellData tile in data.cliff_cells)
                {
                    if (tile == null || tile.cell == null || tile.cell.Length < 3) continue;
                    Vector3Int cell = new Vector3Int(tile.cell[0], tile.cell[1], tile.cell[2]);
                    dto.cliffCells.Add(new RoomData.TileCellRecord
                    {
                        assetGuidOrName = tile.tile_guid,
                        cell = cell,
                        worldPos = new Vector3(cell.x, cell.y, cell.z),
            ...
            if (data?.prop_instances != null)
            {
                foreach (PropData prop in data.prop_instances)
                {
                    if (prop == null) continue;
                    Vector3 pos = ToVector3(prop.position);
                    dto.propPlacements.Add(new RoomData.PropPlacement
                    {
                        assetGuidOrName = prop.prefab_guid,
                        cell = Vector3Int.RoundToInt(pos),
                        position = pos,
                        rotation = prop.rotation,
                        scale = Vector2.one,
                        layer = RoomLayer.Props,
                    });
                }
            }
